#!/bin/bash
# Docker setup script using .env.development file

set -e

echo "===== Docker Setup Using .env.development ====="
...
# Step 3: Create a docker-compose file that uses .env.development
echo "Step 3: Creating docker-compose.env.yml..."
...
volumes:
  ...
  redis-data:
EOF
echo "✅ docker-compose.env.yml created"
...
echo "This Docker setup is using the .env.development file for configuration."
echo "To verify the environment variables in the container, run:"
echo "docker exec redbarsushi-app env | grep -v PATH | sort"

echo
echo "===== Docker Setup Completed ====="
echo "Your Docker environment is now running using the .env.development file."
echo
...
echo "- Normal restart: ./docker_env_setup.sh"
echo "- Clean restart (removes volumes): ./docker_env_setup.sh --clean"