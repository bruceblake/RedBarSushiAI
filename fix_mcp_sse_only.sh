...
echo "===== Fixing MCP Server (SSE-only mode) ====="
...
# Enhance the SSE endpoint in the server code to ensure it works correctly
sed -i '/^@app.route.*mcp.*GET/,/^def mcp_sse_endpoint/c\
@app.route(\"/mcp\", methods=[\"GET\"])\
def mcp_sse_endpoint():' enhanced_mcp_server.py

# Fix the SSE endpoint code to ensure it sends proper SSE messages
sed -i '/def stream():/,/return Response/c\
    def stream():\
