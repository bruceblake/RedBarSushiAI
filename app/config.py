...
try:
    ...
    if pydantic_version >= (2, 0):
        ...
        from pydantic import Field, field_validator as validator  # AnyHttpUrl removed

        PYDANTIC_V2 = True
    else:
        # Pydantic v1 imports
        from pydantic import BaseSettings, Field, validator  # AnyHttpUrl removed
...
logger.critical(
    f"Raw OPENAI_API_KEY directly from os.environ: '{raw_openai_key_from_os_environ}' (Type: {type(raw_openai_key_from_os_environ).__name__})"
)
if raw_openai_key_from_os_environ:
    logger.critical(
        f"Raw OPENAI_API_KEY preview from os.environ: {raw_openai_key_from_os_environ[:7]}...{raw_openai_key_from_os_environ[-4:] if len(raw_openai_key_from_os_environ) > 11 else ''}"
    )
else:
...
class Settings(BaseSettings):
    ...
    # Base configuration
    # APP_NAME: str = "RedBarSushiAI" # Removed based on Vulture and instructions
    ...
    ENVIRONMENT: str = Field(
        "development", env="FLASK_ENV"
    )  # Keep FLASK_ENV for compatibility
    ...
    DATABASE_URL: str = Field(
        "sqlite:///app.db", env="DATABASE_URL"
    )  # Default to SQLite for local dev
    # SQLALCHEMY_TRACK_MODIFICATIONS: bool = False # Removed based on Vulture and instructions
    ...
    OPENAI_API_KEY: Optional[str] = Field(
        "USING_DEFAULT_FALLBACK_KEY", env="OPENAI_API_KEY"
    )
    OPENAI_REALTIME_MODEL: str = Field(
        "gpt-4o-realtime-preview-2024-12-17", env="OPENAI_REALTIME_MODEL"
    )
    ...
    # STRIPE_WEBHOOK_SECRET: Optional[str] = Field(None, env="STRIPE_WEBHOOK_SECRET") # Removed based on Vulture and instructions

    # Deliverect settings
    # DELIVERECT_CHANNEL_NAME: str = Field("redbarsushi", env="DELIVERECT_CHANNEL_NAME") # Removed based on Vulture and instructions
    DELIVERECT_API_KEY: Optional[str] = Field(None, env="DELIVERECT_API_KEY")
    DELIVERECT_BASE_URL: str = Field(
        "https://api.staging.deliverect.com", env="DELIVERECT_BASE_URL"
    )
    DELIVERECT_API_URL: str = Field(
        "https://api.staging.deliverect.com/v2/orders", env="DELIVERECT_API_URL"
    )
    DELIVERECT_CLIENT_ID: Optional[str] = Field(None, env="DELIVERECT_CLIENT_ID")
    DELIVERECT_CLIENT_SECRET: Optional[str] = Field(
        None, env="DELIVERECT_CLIENT_SECRET"
    )
    ...
    TWILIO_CONVERSATION_SERVICE_SID: Optional[str] = Field(
        None, env="TWILIO_CONVERSATION_SERVICE_SID"
    )
    TWILIO_CONNECTOR_NAME: Optional[str] = Field(None, env="TWILIO_CONNECTOR_NAME")

    # Agent IDs
    # OPENAI_FRONTLINE_AGENT_ID: Optional[str] = Field(None, env="OPENAI_FRONTLINE_AGENT_ID") # Removed
    # OPENAI_MENU_AGENT_ID: Optional[str] = Field(None, env="OPENAI_MENU_AGENT_ID") # Removed
    # OPENAI_CART_AGENT_ID: Optional[str] = Field(None, env="OPENAI_CART_AGENT_ID") # Removed
    # OPENAI_FULFILLMENT_AGENT_ID: Optional[str] = Field(None, env="OPENAI_FULFILLMENT_AGENT_ID") # Removed
    # OPENAI_GUARDRAIL_AGENT_ID: Optional[str] = Field(None, env="OPENAI_GUARDRAIL_AGENT_ID") # Removed
    # OPENAI_ESCALATION_AGENT_ID: Optional[str] = Field(None, env="OPENAI_ESCALATION_AGENT_ID") # Removed

    # Database config
    INITIALIZE_MENU_DATABASE: bool = Field(True, env="INITIALIZE_MENU_DATABASE")

    # Optional MCP settings
    # MCP_PORT: Optional[int] = Field(None, env="MCP_PORT") # Removed
    # MCP_URL: Optional[str] = Field(None, env="MCP_URL") # Removed
    ...
    def validate_voice_handler(cls, v, **kwargs):
        ...
        if v not in allowed_values:
            raise ValueError(
                f"Unsupported VOICE_HANDLER value: {v}, allowed values: {allowed_values}"
            )
        return v

    # model_config and Config class removed as per analysis
...
try:
    ...
    if settings.OPENAI_API_KEY:
        logger.critical(
            f"Loaded settings.OPENAI_API_KEY preview: {settings.OPENAI_API_KEY[:7]}...{settings.OPENAI_API_KEY[-4:] if len(settings.OPENAI_API_KEY) > 11 else ''}"
        )
        if "mytestapikey" in settings.OPENAI_API_KEY.lower():
            logger.critical(
                "CRITICAL: OPENAI_API_KEY loaded appears to be a TEST/DUMMY key: "
    ...
    logger.critical(
        f"CRITICAL ERROR during Pydantic Settings instantiation in app.config.py: {e}",
        exc_info=True,
    )
    ...
    try:
        ...
        logger.critical(
            f"Minimal config settings.OPENAI_API_KEY: '{settings.OPENAI_API_KEY}'"
        )
        if settings.OPENAI_API_KEY:
            logger.critical(
                f"Minimal config settings.OPENAI_API_KEY preview: {settings.OPENAI_API_KEY[:7]}...{settings.OPENAI_API_KEY[-4:] if len(settings.OPENAI_API_KEY) > 11 else ''}"
            )
        ...
        logger.critical(
            f"Raw OPENAI_API_KEY before full settings creation: '{raw_key}'"
        )
        ...
        settings = Settings(
            ...
            ),  # EXPLICITLY SET FALLBACK FOR DEBUGGING
            ...
            DELIVERECT_API_URL=os.environ.get(
        ...
        if settings.OPENAI_API_KEY:
            logger.critical(
                f"Final settings.OPENAI_API_KEY preview: {settings.OPENAI_API_KEY[:7]}...{settings.OPENAI_API_KEY[-4:] if len(settings.OPENAI_API_KEY) > 11 else ''}"
            )
            if "mytestapikey" in settings.OPENAI_API_KEY.lower():
                logger.critical(
                    "CRITICAL: Final OPENAI_API_KEY is a TEST/DUMMY key: "
                    + settings.OPENAI_API_KEY
                )
