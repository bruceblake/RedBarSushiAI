...
# helpers_async import removed as commit_with_retry_async and log_info_async are unused
...
class ModificationResponse(BaseModel):
    ...
    updated_order_items: Optional[List[dict]] = Field(
        None, description="The updated order items"
    )
    modifications_applied: Optional[bool] = Field(
        None, description="Whether modifications were applied"
    )
...
async def modify_order(
    request: ModificationRequest, db: AsyncSession = Depends(get_db)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        available_items, unavailable_items = await mark_unavailable_items_async(
            db, updated_order
        )
        ...
        total_price = sum(
            item.get("price", 0) * item.get("quantity", 1) for item in updated_order
        )
