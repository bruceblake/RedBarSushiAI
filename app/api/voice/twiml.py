...
async def receive_call(request: Request) -> PlainTextResponse:
    ...
    try:
        ...
        logger.critical(f"[call_sid] ***** WEBHOOK ENDPOINT ACCESSED SUCCESSFULLY: {request.url.path} *****")
        logger.info(f"[call_sid] ==== INCOMING REALTIME CALL DETAILS ====")
        logger.info(f"[call_sid] Call SID: {call_sid}")
        logger.info(f"[call_sid] Request came from: {request.client.host}")
        logger.info(f"[call_sid] User agent: {request.headers.get('user-agent', 'unknown')}")
        logger.info(f"[call_sid] Host header: {request.headers.get('host', 'unknown')}")
        logger.info(f"[call_sid] URL: {request.url}")
        logger.info(f"[call_sid] Path: {request.url.path}")
        logger.info(f"[call_sid] Request method: {request.method}")
        logger.info(f"[call_sid] Environment: {os.environ.get('FASTAPI_ENV', 'undefined')}")
        logger.info(f"[call_sid] Current working directory: {os.getcwd()}")
