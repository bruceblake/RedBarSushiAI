...
import logging
import time
from typing import Dict, Any, Optional, Tuple  # Callable removed
...
class AsyncAgentOrchestrator:
    ...
    async def initialize(self, db=None):
        ...
        """
        logger.info(
            f"Initializing orchestrator with database session: {db is not None}"
        )
    ...
    async def process_voice_input(
        self, call_sid: str, input_text: str, context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Processed voice input in {duration:.2f}s through FSM state {fsm.current_state} with {agent_name}: {input_text[:50]}..."
        )
        ...
        return {
            ...
            "fsm_context": {
                k: v
                for k, v in fsm.context.items()
                if isinstance(v, (str, int, float, bool, list, dict)) or v is None
            },
        }

    async def _process_with_appropriate_agent(
        self, fsm: AsyncConversationFSM, input_text: str, context: Dict[str, Any]
    ...
    # process_tool_call method removed as unused by Vulture

    # get_session_state method removed as unused by Vulture

    async def start_new_conversation(
        self, call_sid: str, context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        greeting_text = greeting_response.get(
            "text", "Welcome to Red Bar Sushi. How can I assist you today?"
        )
    ...
    # cleanup_inactive_sessions method removed as unused by Vulture
    ...
    async def handle_interruption(self, call_sid: str):
        ...
        fsm = await self.get_fsm(call_sid)
        fsm.update_context(
            {"user_interrupted": True, "last_interruption_time": time.time()}
        )

        # Signal frontline agent about interruption if available
        if self.frontline_agent and hasattr(
            self.frontline_agent, "handle_interruption"
        ):
        ...
        await self.conversation_store.add_message(
            call_sid, role="system", content="[User interrupted TTS playback]"
        )
