...
def migrate_menu_to_database(file_path=None, location_id=None, force=False):
    ...
    try:
        ...
        menu_data = safe_load_json(
            file_path, default={"items": [], "modifiers": [], "modifierGroups": []}
        )
        ...
        if existing_count > 0 and not force:
            logger.warning(
                f"Database already contains {existing_count} menu items. Use force=True to override."
            )
            return {
                "success": False,
        ...
        if result:
            ...
            logger.info(
                f"Successfully migrated menu data: {items_count} items, {modifiers_count} modifiers, {modifier_groups_count} groups"
            )
...
def verify_menu_migration(location_id=None):
    ...
    try:
        ...
        db_menu_data = menu_db_store.get_menu_data(
            location_id=location_id, force_refresh=True
        )
