    ...
    def interactive_order_resolution(
        ...
        try:
            ...
            if session_id:
                ...
                # Check if we actually have messages in the conversation
                if not conversation_data.get("messages"):
                    logger.info(f"[MENU-MATCHER] No messages found in conversation, initializing empty conversation")
                    conversation_data = {
                        "id": session_id,
                    ...
                    # Save the empty conversation to ensure consistency
                    conversation_store.save_conversation(session_id, conversation_data)
                    logger.info(f"[MENU-MATCHER] Saved initialized empty conversation for consistency")
            ...
            if conversation_data and "messages" in conversation_data and conversation_data["messages"]:
                # Only add previous messages if there's actually some content
                ...
                if has_actual_content:
                    ...
                    # Add the new user request with context reference
                    messages.append({
                        "role": "user",
                    ...
                    # No actual message content, treat as new conversation
                    logger.info(f"[MENU-MATCHER] Conversation exists but has no content, treating as new conversation")
                    messages.append({
                        "role": "user",
                    ...
                    # Initialize conversation in the store with this first message
                    if session_id:
