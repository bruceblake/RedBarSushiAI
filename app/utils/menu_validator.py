...
def validate_and_fix_menu_data(menu_data, location_id=None):
    ...
    if (
        ...
        logger.info(
            "[MENU-FORMAT] Detected Deliverect webhook menu format with data.menu structure"
        )
    ...
    # Check for Deliverect async update format with body.menus structure
        ...
        logger.info(
            "[MENU-FORMAT] Detected Deliverect async menu format with body.menus structure"
        )
        # Use the first menu in the menus array
        if (
            isinstance(menu_data["body"]["menus"], list)
            and len(menu_data["body"]["menus"]) > 0
        ):
    ...
    if "categories" in menu_data and isinstance(menu_data["categories"], list):
        logger.info(
            "[MENU-FORMAT] Detected Deliverect categories format, converting to internal structure"
        )
        ...
        for category in menu_data["categories"]:
            ...
            elif "subProducts" in category and isinstance(
                category["subProducts"], list
            ):
            ...
            for product in category_products:
                if isinstance(product, dict):
                    ...
                    product["category_id"] = category.get("_id") or category.get(
                        "id", ""
                    )
                    ...
                    if (
                        product.get("productType") == 3
                        or "subProducts" in product
                        or "subCategories" in product
                    ):
        ...
        if all_items:
            logger.info(
                f"[MENU-FORMAT] Extracted {len(all_items)} products from {len(menu_data['categories'])} categories"
            )
            menu_data["items"] = all_items

    # Handle case where menu data has products but not items
    elif (
        "products" in menu_data
        and isinstance(menu_data["products"], (list, dict))
        and "items" not in menu_data
    ):
    ...
    if isinstance(menu_data.get("modifierGroups"), dict):
        logger.info(
            "[MENU-FORMAT] Converting Deliverect modifierGroups dictionary to list"
        )
    ...
    if "modifierGroups" not in menu_data or not isinstance(
        ...
        for i, group in enumerate(menu_data["modifierGroups"]):
            if isinstance(group, dict):
                ...
                if (
                    "subProducts" in group
                    and isinstance(group["subProducts"], list)
                    and not group.get("modifiers")
                ):
    ...
    for i, item in enumerate(menu_data.get("items", [])):
        ...
        if not item_id:
            ...
            # Check if plu exists (Deliverect format often uses plu as primary reference)
                ...
                logger.info(
                    f"[MENU-FIX] Using plu as id for item index {i}: {item['id']}"
                )
                item_id = item["id"]
                item_fixed = True
            else:
        ...
        # 3. Has subProducts but no price - likely a category
            ...
            "subProducts" in item
            and isinstance(item.get("subProducts"), list)
            and len(item.get("subProducts")) > 0
        ):
            is_category = True
        # 4. From the categories section with the same ID structure
        elif (
            item.get("_id")
            and "categories" in menu_data
            and isinstance(menu_data["categories"], list)
        ):
            for category in menu_data["categories"]:
        ...
        elif isinstance(item.get("name"), str) and item.get("name").startswith(
            "[CATEGORY]"
        ):
            is_category = True
        # 6. Use contextual information to identify categories based on patterns, not hardcoded values
            ...
            if (
                ("categor" in item_name_lower)
                or ("section" in item_name_lower)
                or ("group" in item_name_lower)
                or (
                    len(menu_data.get("categories", [])) > 0
                    and any(
                        category.get("name", "").lower() == item_name_lower
        ...
        if item.get("isVariant") == True:
            ...
            logger.info(
                f"[MENU-FIX] Item {item_name} identified as a variant product with isVariant flag"
            )
        # Check if it has modifiers or subProducts with prices
        elif (
            "subItems" in item
            and isinstance(item.get("subItems"), list)
            and len(item.get("subItems")) > 0
        ):
            is_variant_product = True
            logger.info(
                f"[MENU-FIX] Item {item_name} identified as a variant product with subItems"
            )
        ...
        # Variant products in Deliverect often have zero price with variants as separate products
            ...
            item.get("price") == 0
            and "subProducts" in item
            and isinstance(item.get("subProducts"), list)
            and len(item.get("subProducts")) > 0
        ):
            is_variant_product = True
            logger.info(
                f"[MENU-FIX] Item {item_name} identified as variant container with subProducts and zero price"
            )
        ...
        if is_category:
            ...
            if "price" not in item or item["price"] is None:
                item["price"] = 0
                logger.info(
                    f"[MENU-FIX] Item {item_name} identified as a category, setting zero price"
                )
                item_fixed = True
            ...
            if "price" not in item or item["price"] is None:
                item["price"] = 0
                logger.info(
                    f"[MENU-FIX] Item {item_name} identified as a variant product, allowing zero price"
                )
                item_fixed = True
                ...
                try:
                    ...
                    logger.info(
                        f"[MENU-FIX] Converted non-numeric price for variant {item_name} to {item['price']}"
                    )
                    item_fixed = True
                    ...
                    item["price"] = 0
                    logger.info(
                        f"[MENU-FIX] Reset invalid price for variant {item_name} to zero"
                    )
                    item_fixed = True
            ...
            # Check if price is not a number
                ...
                try:
                    ...
                    item["price"] = float(item["price"])
                    logger.info(
                        f"[MENU-FIX] Converted non-numeric price for {item_name} to {item['price']}"
                    )
                except (ValueError, TypeError):
                    price_invalid = True
                    logger.info(
                        f"[MENU-FIX] Item {item_name} has invalid non-numeric price: {item.get('price')}"
                    )
            # Check if price is negative
            elif item["price"] < 0:
                price_invalid = True
                logger.info(
                    f"[MENU-FIX] Item {item_name} has negative price: {item['price']}"
                )
            # For zero prices, check if this might be a variant container
                ...
                if (
                    ...
                    and (
                        "###" in item.get("plu")
                        or "#" in item.get("plu")
                        or "-PRNT" in item.get("plu")
                        or "-VAR" in item.get("plu")
                    )
                ) or (
                    item.get("name")
                    and any(
                    ...
                    logger.info(
                        f"[MENU-FIX] Allowing zero price for apparent variant container: {item_name}"
                    )
                    # Don't set price_invalid - leave as valid
                else:
                    price_invalid = True
                    logger.info(
                        f"[MENU-FIX] Item {item_name} has zero price: {item['price']}"
                    )
            ...
            if price_invalid:
                ...
                if "referenceId" in item and item.get("referenceId"):
                    ...
                    logger.info(
                        f"[MENU-FIX] Found referenceId {reference_id} for {item_name}, looking for base product"
                    )
                    ...
                    for other_item in menu_data.get("items", []):
                        if (
                            other_item.get("plu") == reference_id
                            or other_item.get("reference_handler") == reference_id
                            or other_item.get("reference_price_source") == reference_id
                        ):
                            if (
                                isinstance(other_item.get("price"), (int, float))
                                and other_item.get("price") > 0
                            ):
                    ...
                    if base_item:
                        ...
                        logger.info(
                            f"[MENU-FIX] Set price for {item_name} from base product with referenceId {reference_id}: {item['price']}"
                        )
                        price_invalid = False
                ...
                # Second check: Handle PLUs with ### pattern (per Deliverect docs)
                    ...
                    if base_plu:
                        logger.info(
                            f"[MENU-FIX] Extracted base PLU {base_plu} from {item.get('plu')} for {item_name}"
                        )
                        ...
                        for other_item in menu_data.get("items", []):
                            if (
                                ...
                                or other_item.get("reference_handler") == base_plu
                            ):
                                if (
                                    isinstance(other_item.get("price"), (int, float))
                                    and other_item.get("price") > 0
                        ...
                        if base_item:
                            item["price"] = base_item.get("price")
                            logger.info(
                                f"[MENU-FIX] Set price for {item_name} from base product with PLU {base_plu}: {item['price']}"
                            )
                            price_invalid = False
                ...
                # Third check: Similar for variants with base_plu already extracted
                    ...
                    logger.info(
                        f"[MENU-FIX] Using pre-extracted base PLU {base_plu} for {item_name}"
                    )
                ...
                # Fourth check: Special case for original_plu with ### (similar to second check)
                    ...
                    logger.info(
                        f"[MENU-FIX] Looking for base product with original_plu base {base_plu} for {item_name}"
                    )
                    ...
                    for other_item in menu_data.get("items", []):
                        if (
                            other_item.get("plu") == extracted_base
                            or other_item.get("reference_handler") == extracted_base
                            or other_item.get("plu") == base_plu
                        ):
                            if (
                                isinstance(other_item.get("price"), (int, float))
                                and other_item.get("price") > 0
                                ...
                                logger.info(
                                    f"[MENU-FIX] Set price for {item_name} from product with PLU {extracted_base}: {item['price']}"
                                )
                                price_invalid = False
                                break
                ...
                if (
                    price_invalid
                    and item_name_lower in existing_items
                    and existing_items[item_name_lower].get("price")
                ):
                ...
                if price_invalid:
                    ...
                    lookup_location_id = (
                        item_location_id if item_location_id else location_id
                    )
                    if lookup_location_id:
                        logger.info(
                            f"[MENU-FIX] Using location_id {lookup_location_id} for database lookup of {item_name}"
                        )

                    menu_data_db = menu_db_store._get_menu_data_from_db(
                        location_id=lookup_location_id,
                        cache_key=f"menu:{lookup_location_id if lookup_location_id else 'default'}",
                    )
                    ...
                    if not menu_data_db.get("items"):
                        ...
                        logger.warning(
                            f"[MENU-FIX] Empty database detected - using special initialization rules for {item_name}"
                        )
                        ...
                        if item.get("raw_price") is not None:
                            ...
                            logger.info(
                                f"[MENU-FIX] Empty DB: Using raw_price from original data for {item_name}: {original_price}"
                            )
                        # If price is in cents format from Deliverect, convert it
                            ...
                            logger.info(
                                f"[MENU-FIX] Empty DB: Normalized cents price for {item_name}: {original_price}"
                            )
                        ...
                        # If this is a variant product, it's valid to have zero price
                            ...
                            item["price"] = 0
                            logger.info(
                                f"[MENU-FIX] Empty DB: Setting zero price for variant container {item_name}"
                            )
                            price_invalid = False
                        # For items with PLU variants, we need to get price from the base product
                            ...
                            base_plu = (
                                item.get("base_plu")
                                if item.get("base_plu")
                                else item.get("plu").split("###")[0]
                            )
                            ...
                            for other_item in menu_data.get("items", []):
                                if (
                                    other_item.get("plu") == base_plu
                                    or other_item.get("reference_handler") == base_plu
                                    or other_item.get("reference_price_source")
                                    == base_plu
                                ):
                            ...
                            if (
                                base_product
                                and "price" in base_product
                                and isinstance(base_product["price"], (int, float))
                                and base_product["price"] > 0
                            ):
                                item["price"] = base_product["price"]
                                logger.info(
                                    f"[MENU-FIX] Empty DB: Using base product price for {item_name}: {item['price']}"
                                )
                                price_invalid = False
                            # If base product has a raw_price, use that instead
                            elif (
                                base_product
                                and "raw_price" in base_product
                                and isinstance(base_product["raw_price"], (int, float))
                                and base_product["raw_price"] > 0
                            ):
                                item["price"] = base_product["raw_price"]
                                logger.info(
                                    f"[MENU-FIX] Empty DB: Using base product raw_price for {item_name}: {item['price']}"
                                )
                                price_invalid = False
                            ...
                            # EMPTY DB SPECIAL CASE - For variant containers, just accept the zero price
                                ...
                                item["price"] = 0
                                logger.warning(
                                    f"[MENU-FIX] Empty DB: Allowing zero price for variant with ### pattern: {item_name} with PLU {item.get('plu')}"
                                )
                                price_invalid = False
                        ...
                        if price_invalid:
                            ...
                            if item.get("productType") == 3 or item.get("is_variant"):
                                item["price"] = 0
                                logger.info(
                                    f"[MENU-FIX] Empty DB: Setting zero price for variant/category {item_name}"
                                )
                                price_invalid = False
                            # For specific pattern PLUs, allow zero prices (such as variant containers from Deliverect)
                            elif "plu" in item and (
                                item["plu"].startswith("P-")  # Any product pattern
                                or item["plu"].startswith("BURG")  # Any burger pattern
                                or item["plu"].startswith("PRT")  # PRT pattern
                                or item["plu"].startswith("BS-")  # Base pattern
                                or item["plu"].startswith("VAR")  # Variant pattern
                                or item["plu"].startswith("DRNK")  # Drink pattern
                                or item["plu"].startswith("XTRA")  # Extra pattern
                            ):
                                ...
                                logger.warning(
                                    f"[MENU-FIX] Empty DB initialization case - accepting item {item_name} with PLU {item['plu']} and price {item['price']}"
                                )
                                price_invalid = False
                            # Allow zero prices for any items that match known variant formatting patterns
                            elif item.get("plu") and (
                                "###" in item.get("plu")  # Deliverect variant pattern
                                or "#" in item.get("plu")  # Common variant marker
                                or "-PRNT"
                                in item.get("plu")  # Variant parent indicator
                                or "-" in item.get("plu")  # Any dash indicator
                                or item.get("plu").endswith(
                                    "-V"
                                )  # Another variant pattern
                                or item.get("plu").endswith(
                                    "-P"
                                )  # Another parent pattern
                                or "-VAR" in item.get("plu")  # Common variant indicator
                                or (
                                    item.get("original_plu")
                                    and "###" in item.get("original_plu")
                                )
                            ):
                                # These match known variant container patterns used by POS systems
                                item["price"] = 0
                                logger.warning(
                                    f"[MENU-FIX] Empty DB: Allowing zero price for detected variant container {item_name} with PLU pattern"
                                )
                                price_invalid = False
                            # Also allow zero prices for items with references to variants in the name
                            elif item.get("name") and any(
                                variant_term in item.get("name", "").lower()
                                for variant_term in [
                                    "variant",
                                    "option",
                                    "container",
                                ...
                                item["price"] = 0
                                logger.warning(
                                    f"[MENU-FIX] Empty DB: Allowing zero price for likely variant container based on name: {item_name}"
                                )
                                price_invalid = False
                            # EMPTY DB FALLBACK - For database initialization, accept anything with a PLU
                                ...
                                item["price"] = 0
                                logger.warning(
                                    f"[MENU-FIX] Empty DB FALLBACK: Allowing zero price for item {item_name} with PLU {item.get('plu')}"
                                )
                                price_invalid = False
                            else:
                    ...
                    for db_item_entry in menu_data_db.get("items", []):
                        ...
                        if lookup_location_id:
                            ...
                            if hasattr(db_item_entry, "location_id"):
                                db_entry_location = getattr(
                                    db_item_entry, "location_id", None
                                )
                            elif (
                                isinstance(db_item_entry, dict)
                                and "location_id" in db_item_entry
                            ):
                        ...
                        if (
                            db_item_name == item_name_lower
                            or db_item_name in item_name_lower
                            or item_name_lower in db_item_name
                        ):
                            db_item = db_item_entry
                            logger.info(
                                f"[MENU-FIX] Found matching item in database: {db_item_name}"
                            )
                            break

                    # If not found by name, try by PLU/reference_handler
                    if not db_item and (
                        item.get("plu") or item.get("reference_handler")
                    ):
                        for db_item_entry in menu_data_db.get("items", []):
                            # If item has location_id, make sure we only match items with same location_id
                            if lookup_location_id:
                            ...
                            if db_item_entry.get("plu") == item.get(
                                "plu"
                            ) or db_item_entry.get("reference_handler") == item.get(
                                "reference_handler"
                            ):
                                db_item = db_item_entry
                                logger.info(
                                    f"[MENU-FIX] Found matching item in database by PLU/reference: {db_item_entry.get('name')}"
                                )
                                break
                    ...
                    if (
                        not db_item
                        and item.get("plu")
                        and ("#" in item.get("plu") or "-" in item.get("plu"))
                    ):
                        plu = item.get("plu")
                        ...
                        if "###" in plu:
                            ...
                            if len(parts) > 1:
                                possible_base_plu = (
                                    "-".join(parts[:-1]) if len(parts) > 2 else parts[0]
                                )

                        if possible_base_plu:
                            logger.info(
                                f"[MENU-FIX] Extracted possible base PLU {possible_base_plu} from {plu} for database search"
                            )
                            for db_item_entry in menu_data_db.get("items", []):
                                # If item has location_id, make sure we only match items with same location_id
                                if lookup_location_id:
                                    ...
                                    if hasattr(db_item_entry, "location_id"):
                                        ...
                                        db_entry_location = db_item_entry.get(
                                            "location_id"
                                        )
                                ...
                                if (
                                    db_item_entry.get("plu") == possible_base_plu
                                    or db_item_entry.get("reference_handler")
                                    == possible_base_plu
                                ):
                                    db_item = db_item_entry
                                    logger.info(
                                        f"[MENU-FIX] Found matching item in database by base PLU: {db_item_entry.get('name')}"
                                    )
                                    break

                    if (
                        db_item
                        and isinstance(db_item.get("price"), (int, float))
                        and db_item.get("price") > 0
                    ):
                        ...
                        menu_data_db_check = menu_db_store._get_menu_data_from_db(
                            cache_key="menu:default"
                        )
                        if not menu_data_db_check.get("items"):
                            logger.warning(
                                f"[MENU-FIX] EMPTY DB VALIDATION: Special handling for {item_name} during database initialization"
                            )
                            ...
                            if item.get("plu"):
                                ...
                                logger.warning(
                                    f"[MENU-FIX] EMPTY DB INITIALIZATION: Allowing item {item_name} with PLU {item.get('plu')}"
                                )
                                price_invalid = False
                            else:
    ...
    for i, modifier in enumerate(modifiers):
        ...
        if price_invalid:
            ...
            for db_mod in menu_data_db.get("modifiers", []):
                ...
                if (
                    db_mod_name == mod_name_lower
                    or db_mod_name in mod_name_lower
                    or mod_name_lower in db_mod_name
                ):
                    db_modifier = db_mod
                    break
