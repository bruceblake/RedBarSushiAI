...
# AsyncMenuMatcher, AsyncCachedMenuMatcher, get_cached_async_menu_matcher removed as unused
from app.utils.snooze_validator import (
    is_item_available,
)  # validate_items_availability removed

logger = logging.getLogger(__name__)

# Menu search and matching functions

# find_menu_item_async function removed as unused
...
async def build_order_description_async(order_items: List[Dict[str, Any]]) -> str:
    ...
    for item in order_items:
        ...
        if modifier_descriptions:
            if quantity > 1:
                descriptions.append(
                    f"{quantity}x {name} with {', '.join(modifier_descriptions)}"
                )
            else:
...
async def mark_unavailable_items_async(
    db: AsyncSession, order_items: List[Dict[str, Any]]
...
async def validate_modifiers_async(
    ...
    for item in order_items:
        ...
        for group in menu_data.get("modifier_groups", []):
            ...
            if "subProducts" in group and menu_item.get(
                "reference_handler"
            ) in group.get("subProducts", []):
        ...
        for modifier in item.get("modifiers", []):
            ...
            for group in item_groups:
                if menu_modifier.get("reference_handler") in group.get(
                    "subProducts", []
                ):
        ...
        for group_id, group_data in modifiers_by_group.items():
            ...
            if count < min_required:
                errors.append(
                    f"Item '{item_name}' requires at least {min_required} selection(s) from '{group.get('name')}'"
                )
            ...
            if max_allowed is not None and count > max_allowed:
                errors.append(
                    f"Item '{item_name}' allows maximum {max_allowed} selection(s) from '{group.get('name')}'"
                )
