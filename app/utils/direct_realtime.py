...
try:
    ...
    if not WEBSOCKETS_AVAILABLE:
        logger.error(
            "Neither websockets nor aiohttp is available - WebSocket functionality will be limited"
        )
    else:
        ...
        try:
            ...
            if WEBSOCKETS_AVAILABLE:
                try:
                    ...
                    logger.info(
                        "WebSocket connection established using websockets library"
                    )
            ...
            if not self.websocket and AIOHTTP_AVAILABLE:
                try:
                    ...
                    logger.info(
                        "WebSocket connection established using aiohttp library"
                    )
            ...
            if not self.websocket and not self._aiohttp_ws:
                raise RuntimeError(
                    "Failed to connect using any available WebSocket backend"
                )
            ...
            while not session_created and time.time() - start_time < timeout:
                try:
                    ...
                    logger.debug(
                        f"Received event while waiting for session.created: {event.get('type')}"
                    )
                    ...
                    if event.get("type") == "session.created":
                        ...
                        logger.info(
                            f"Session created successfully with ID: {self.session_id}"
                        )
                    elif event.get("type") == "error" or event.get("type").startswith(
                        "invalid_"
                    ):
                        logger.error(
                            f"Error event received during session creation: {event}"
                        )
                        raise ConnectionError(
                            f"Error creating session: {event.get('message', 'Unknown error')}"
                        )
            ...
            if self.backend == "websockets":
                ...
                async for msg in self._aiohttp_ws:
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        ...
                        logger.error(
                            f"WebSocket connection error (aiohttp): {msg.data}"
                        )
                        break
            ...
            try:
                ...
                async for chunk in audio_chunks_generator:
                    if isinstance(chunk, bytes):
                        ...
                        if chunks_sent % 10 == 0:
                            logger.debug(
                                f"Sent {chunks_sent} audio chunks to WebSocket"
                            )

                # Signal that we're done sending audio
                logger.info(
                    f"Done collecting audio chunks, sent {chunks_sent} chunks total"
                )
...
- Pronounce any Japanese food terms correctly
            ...
            try:
                ...
                logger.info(
                    f"Created realtime conversation session: {session.session_id}"
                )
            except Exception as session_error:
                logger.error(
                    f"Error creating session for conversation: {session_error}"
                )
                # Fall back to streaming standard API
                logger.warning(
                    "Falling back to standard conversation API with streaming"
                )

                # Add system message if not present
                if not any(msg.get("role") == "system" for msg in conversation_history):
                    conversation_history.insert(
                        0, {"role": "system", "content": restaurant_system_prompt}
                    )
            ...
            try:
                ...
                for msg in conversation_history:
                    if (
                        msg.get("role") != "system"
                    ):  # Skip system message as we already added it
                        await session.send_event(
                            {
                                "type": "conversation.item.create",
                                "item": {
...
async def _post_process_transcript(self, transcript):
    ...
    try:
        ...
        menu_items_str = ", ".join(
            menu_items[:20] if len(menu_items) > 20 else menu_items
        )
...
def process_audio(audio_data, callback=None):
    ...
    try:
        ...
        error_result = {"type": "error", "error": error_msg, "timestamp": time.time()}
