...
async def process_deliverect_menu_async(menu_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "categories" in menu_data:
        for category in menu_data["categories"]:
            ...
            cat_data = {
                ...
                "sub_products": category.get("subProducts", [])  # Order of products in category
            }
    ...
    if "products" in menu_data:
        for product_id, product in menu_data["products"].items():
            ...
            item_data = {
                ...
                "is_available": product.get("visible", True) and not product.get("snoozed", False),
                "is_combo": product.get("isCombo", False),
                "is_variant": product.get("isVariant", False),
                "image_url": product.get("imageUrl", ""),
                "delivery_tax": product.get("deliveryTax", 0) / 1000,  # Convert to percentage
                "takeaway_tax": product.get("takeawayTax", 0) / 1000,  # Convert to percentage
                "eat_in_tax": product.get("eatInTax", 0) / 1000,  # Convert to percentage
            ...
            for mod_group_id in product.get("subProducts", []):
                if mod_group_id in modifier_group_map:
                    item_data["modifier_groups"].append(modifier_group_map[mod_group_id])
    ...
    if "snoozedProducts" in menu_data:
        for snooze_key, snooze_data in menu_data["snoozedProducts"].items():
    ...
    logger.info(f"Processed {len(processed['items'])} items, "
