"""
Redis-based menu caching for RedBarSushiAI using FastAPI.
...
def get_redis_client():
    ...
    try:
        ...
        try:
            ...
            logger.info("Using Redis URL from settings")
        except (ImportError, AttributeError) as e:
            logger.warning(f"Could not import settings: {e}")
            # Fall back to environment variable
            redis_url = os.environ.get("REDIS_URL")
            logger.info("Using Redis URL from environment variables")
        
        if redis_url:
            # Mask credentials in log
            log_url = redis_url.split('@')[-1] if '@' in redis_url else 'redis://localhost'
            logger.info(f"Creating Redis client with URL: {log_url}")
        ...
        if is_render:
            ...
            # On Render without explicit Redis URL, try internal Redis
            return redis.Redis(host="localhost", port=6379, db=0)
        
        # Last resort - try Docker Redis first, then localhost
        for host in ["redis", "localhost", "127.0.0.1"]:
            try:
                logger.info(f"Trying Redis connection to {host}:6379")
                client = redis.Redis(host=host, port=6379, db=0, socket_timeout=1.0)
...
class MenuCache:
    ...
    def initialize_redis(self):
        ...
        try:
            # Direct Redis initialization with improved connection handling
            ...
            if self.redis_client:
                ...
                logger.warning("Failed to get Redis client, menu cache will be disabled")
                self.initialized = False
