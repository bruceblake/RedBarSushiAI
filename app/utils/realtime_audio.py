...
if "X11_SETUP_SUCCESS" in os.environ and os.environ.get("X11_SETUP_SUCCESS") == "true":
    ...
    try:
        ...
        logging.warning(
            f"X11 setup claims success but display {display_env} is not responding, forcing headless mode"
        )
        if "DISPLAY" in os.environ:
            del os.environ["DISPLAY"]
        os.environ["PYNPUT_HEADLESS"] = "1"
...
try:
    ...
    try:
        ...
        try:
            ...
            logging.info(
                "Attempting to import RealtimeClient from openai_realtime_client..."
            )

            # Try to import RealtimeClient, being extra careful with X11 errors
            if not display_env:
                logging.warning(
                    "No DISPLAY environment variable - this will likely fail"
                )
            ...
            logging.info(
                f"✅ Successfully imported RealtimeClient from openai_realtime_client v{version}"
            )
            ...
            try:
                ...
                logging.info(
                    f"✅ Successfully created test client object: {test_client}"
                )
            ...
            if (
                ...
                or "x11" in error_str
                or "x server" in error_str
                or "displaynameerror" in error_str
            ):
                logging.warning(
                    f"X11/Display error importing RealtimeClient: {client_error}"
                )
                # Explicitly set to use direct implementation
                REALTIME_AVAILABLE = False
                logging.warning(
                    "X11/Display dependency detected, will use direct WebSocket implementation"
                )
            else:
        ...
        REALTIME_AVAILABLE = False
        logging.warning(
            f"Could not import RealtimeClient from openai_realtime_client: {e}"
        )
...
if not WEBSOCKET_CAPABILITY:
    logging.warning(
        "No WebSocket capability available (neither openai_realtime_client, websockets, nor aiohttp). Falling back to standard API"
    )
else:
    if not REALTIME_AVAILABLE:
        if WEBSOCKETS_AVAILABLE or AIOHTTP_AVAILABLE:
            logging.info(
                "Will use direct WebSocket implementation with websockets or aiohttp"
            )

            # Don't treat this as an error - it's expected behavior with our dual approach
            if (
                "X11_SETUP_SUCCESS" in os.environ
                and os.environ.get("X11_SETUP_SUCCESS") == "true"
            ):
                logging.info(
                    "Note: Using direct WebSocket implementation instead of OpenAI Realtime client, "
                    + "even though X11 is available. This is normal if the Session class is not available."
                )
            else:
                logging.info(
                    "Using direct WebSocket implementation because X11 display server is not available"
                )
        else:
            logging.warning(
                "No WebSocket libraries available, falling back to standard API"
            )

OPENAI_STREAMING_AVAILABLE = True
logging.info(
    "Using direct OpenAI API for audio streaming without pynput (headless mode)"
)
...
class BasicAudioProcessor:
    ...
    def __init__(self):
        ...
        if self.openai_client is None and (
            os.environ.get("TESTING") == "True"
            or os.environ.get("DISABLE_OPENAI") == "True"
        ):
    ...
    def generate_speech(self, text: str, voice: str = "alloy") -> bytes:
        ...
        try:
            logger.info(
                f"Generating speech for text: '{text[:50]}...' using voice: {voice}"
            )
    ...
    async def process_audio_stream(
        ...
        try:
            ...
            try:
                ...
                session_options = {
                    "input_audio_format": {"type": content_type},
                    "output_audio_format": {"type": "audio/mp3"},
                }

                logger.info(
                    f"Created realtime client with session options: {session_options}"
                )
            except Exception as client_error:
            ...
            try:
                ...
                base64.b64encode(all_audio).decode("utf-8")
                ...
                logger.warning(
                    "Falling back to non-streaming audio processing due to processing error"
                )
            ...
            try:
                ...
                try:
                    ...
                    with tempfile.NamedTemporaryFile(
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Direct RealtimeClient.process_audio_file not available: {direct_error}"
                            )
                    ...
                    logger.error(
                        f"Error processing with RealtimeClient: {process_error}"
                    )
                    logger.error(traceback.format_exc())
                ...
                yield {
                    "type": "error",
                    "error": f"Error processing session events: {str(event_error)}",
                }
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Direct RealtimeClient.generate_speech not available: {direct_error}"
                    )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # Add system message if not present
                    if not any(
                        msg.get("role") == "system" for msg in conversation_history
                    ):
                        messages.append(
                            {
                                "role": "system",
                    ...
                    for msg in conversation_history:
                        if (
                            msg.get("role") != "system" or not messages
                        ):  # Add system if we didn't add one above
                            messages.append(msg)
                    ...
                    try:
                        ...
                        logger.info(
                            "Used RealtimeClient.chat.completions.create streaming method"
                        )
                        return
                    except (AttributeError, TypeError) as method_error:
                        logger.warning(
                            f"RealtimeClient.chat.completions.create not available: {method_error}"
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"RealtimeClient.create_conversation not available: {method_error}"
                        )

                    # Fall back to standard API if we got here
                    logger.warning(
                        "No RealtimeClient methods available, falling back to standard API"
                    )
                except Exception as api_error:
                    logger.error(f"Error using RealtimeClient API: {api_error}")
                    logger.warning(
                        "Falling back to standard API due to RealtimeClient API error"
                    )
            except Exception as client_error:
                logger.error(f"Error creating or using RealtimeClient: {client_error}")
...
def get_audio_processor():
    ...
    if WEBSOCKETS_AVAILABLE or AIOHTTP_AVAILABLE:
        try:
            ...
            if WEBSOCKETS_AVAILABLE and AIOHTTP_AVAILABLE:
                logger.info(
                    "Using DirectRealtimeAudioProcessor with both websockets and aiohttp support"
                )
            elif WEBSOCKETS_AVAILABLE:
                logger.info(
                    "Using DirectRealtimeAudioProcessor with websockets support"
                )
            else:
            ...
            logger.error(
                f"Error initializing DirectRealtimeAudioProcessor: {direct_error}"
            )
            logger.error(traceback.format_exc())
    else:
        logger.warning(
            "Neither websockets nor aiohttp available, skipping DirectRealtimeAudioProcessor"
        )
    ...
    if REALTIME_AVAILABLE:
        try:
            ...
            logger.info(
                "Using RealtimeAudioProcessor with official OpenAI Realtime client"
            )
            return processor
    ...
    try:
        ...
        class MinimalAudioProcessor:
            ...
            async def process_audio(self, audio_data, content_type="audio/webm"):
                ...
                if self.test_mode:
                    logger.info(
                        "MinimalAudioProcessor returning mock transcription in test mode"
                    )
                    return {
                        "type": "transcript_complete",
                        "text": "Mock transcription for testing",
            ...
            async def process_audio_stream(
                self, audio_chunks_generator, content_type="audio/webm"
            ):
                ...
                try:
                    ...
                    yield {
                        "type": "error",
                        "error": "Failed to collect audio chunks",
                        "timestamp": time.time(),
                    }
                    return
            ...
            async def generate_speech(self, text, voice="alloy"):
                ...
                if self.test_mode:
                    logger.info(
                        "MinimalAudioProcessor returning mock audio in test mode"
                    )
            ...
            async def process_conversation(self, transcript, conversation_history=None):
                ...
                if self.test_mode:
                    logger.info(
                        "MinimalAudioProcessor returning mock conversation in test mode"
                    )
...
def process_chunk(chunk_data, session_id, callback=None, continue_existing=False):
    ...
    if (
        ...
        result = {
            "type": "transcript",
            "text": (
                f"Mock response for: {chunk_data_str[:20]}..."
                if chunk_data_str
                else "Mock response for empty input"
            ),
    ...
    try:
        ...
        user_message = (
            chunk_data.decode("utf-8") if isinstance(chunk_data, bytes) else chunk_data
        )
