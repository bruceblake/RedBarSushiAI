...
class AsyncAgentFactory:
    ...
    def __init__(self):
        ...
        # The rule-based frontline agent (AsyncFrontlineVoiceAgent) was an alias and has been removed.
        # We now directly use AsyncFrontlineVoiceAgentAI.
        self.register_agent_class("frontline", AsyncFrontlineVoiceAgentAI)
        logger.info("Using AI-enhanced frontline agent (AsyncFrontlineVoiceAgentAI)")

        # Use enhanced menu agent if AI is enabled
        use_ai_agents = getattr(settings, "USE_AI_AGENTS", True) # Define use_ai_agents
        if use_ai_agents:
    ...
    async def get_agent(
        self, agent_type: str, agent_id: Optional[str] = None, db=None
    ) -> BaseAsyncAgent:
        """
