...
class AsyncGuardrailAgent(BaseAsyncAgent):
    ...
    async def initialize(self):
        ...
        logger.info("AsyncGuardrailAgent: Initializing resources")
    ...
    async def validate_order(
        ...
        """
        logger.info(
            f"[{call_sid}] AsyncGuardrailAgent: Validating order: {order_details}"
        )
        ...
        for item in items:
            ...
            if quantity <= 0:
                ...
                validation_issues.append(
                    f"Invalid quantity for {item.get('name', 'unknown item')}."
                )
    ...
    async def process_input(
        ...
        logger.info(
            f"[{call_sid}] AsyncGuardrailAgent process_input called. Input: '{input_text}'"
        )
