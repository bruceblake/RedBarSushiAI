...
import logging
import json
from typing import Dict, Any, Optional  # Callable, Set removed
...
# FSMError class removed as unused
...
class AsyncStateHandler:
    ...
    async def handle_event(
        self, event: ConversationEvent, context: Dict[str, Any]
...
class AsyncConversationFSM:
    ...
    async def trigger(self, event: ConversationEvent) -> None:
        ...
        if (
            ...
            or event not in self.transitions[self.current_state]
        ):
    ...
    async def transition_to(self, next_state: ConversationState) -> None:
        ...
        # previous_state = self.current_state # Removed as unused
        ...
        if next_handler:
            ...
            if pending_event:
                logger.info(
                    f"Found pending event {pending_event} after entering {next_state}"
                )
    ...
    async def _save_state(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Saved FSM state {self.current_state} for call {self.call_sid}"
            )
    ...
    async def load_state(self) -> None:
        ...
        try:
            ...
            conversation = await async_conversation_store.get_conversation(
                self.call_sid
            )
            ...
            if "fsm_state" in conversation:
                ...
                try:
                    ...
                    logger.info(
                        f"Loaded FSM state {self.current_state} for call {self.call_sid}"
                    )
                except KeyError:
                    logger.warning(
                        f"Unknown FSM state {state_name}, defaulting to INITIAL"
                    )
                    self.current_state = ConversationState.INITIAL
    ...
    async def _extract_event_from_transcript(
        self, transcript: str
        ...
        try:
            ...
            if event:
                logger.info(
                    f"LLM detected event {event.name} from transcript: '{transcript[:50]}...'"
                )
            else:
                logger.info(
                    f"No event detected by LLM for transcript: '{transcript[:50]}...'"
                )
...
class AsyncFSMManager:
    ...
    # cleanup_all method removed as unused
