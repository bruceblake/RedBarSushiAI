...
from fastapi.staticfiles import StaticFiles # Moved up
import uvicorn

from app.config import settings
from app.api import api_router # Moved up
...
# from fastapi.staticfiles import StaticFiles # Moved up
# import os # Redundant import os removed
...
try:
    ...
    logger.info(
        "Successfully registered WebSocket router for /realtime/ws/media/{call_sid}"
    )
...
async def startup_event():
    ...
    logger.critical(
        "❗❗❗ WebSocket route should be available at: /realtime/ws/media/{call_sid} ❗❗❗"
    )
    logger.critical(
        "❗❗❗ TwiML route should be available at: /voice/ and /voice/webhook ❗❗❗"
    )
    ...
    try:
        ...
        async for db in get_db():
            ...
            logger.info(
                "Async agent orchestrator initialized successfully with database session"
            )
            break
    except Exception as e:
