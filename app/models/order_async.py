...
class Order(Base, TimestampMixin):
    ...
    deliverect_channel_order_id: Mapped[Optional[str]] = mapped_column(
...
class OrderItemModifier(Base, TimestampMixin):
    ...
    order_item_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("order_items.id"), nullable=False
    )
...
class ContactRequest(Base, TimestampMixin):
    ...
    # customer_email: Mapped[Optional[str]] = mapped_column(String(255), nullable=True) # Removed as unused
    message: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    request_type: Mapped[str] = mapped_column(
        String(50)
    )  # callback, menu_notification, etc.
