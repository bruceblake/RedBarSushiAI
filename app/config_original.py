...
# Import Pydantic - we'll use v1 style which works with both versions
try:
    # Try Pydantic v1 style first (works with pydantic<2.0)
    from pydantic import BaseSettings, Field, validator
    PYDANTIC_V2 = False
except ImportError:
    try:
        # Try Pydantic v2 style (requires pydantic>=2.0 and pydantic-settings)
        from pydantic_settings import BaseSettings
        from pydantic import Field, field_validator as validator
        PYDANTIC_V2 = True
    except ImportError:
        # Final fallback - create a simple BaseSettings class
        logger.warning("Pydantic not properly installed, using fallback")
        BaseSettings = object
...
class Settings(BaseSettings):
    ...
    # Voice config - ConversationRelay only
    # System uses ConversationRelay exclusively for voice handling
    ...
    CONVERSATION_RELAY_TTS_PROVIDER: str = Field("ElevenLabs", env="CONVERSATION_RELAY_TTS_PROVIDER")
    CONVERSATION_RELAY_TTS_VOICE: Optional[str] = Field(None, env="CONVERSATION_RELAY_TTS_VOICE")
    CONVERSATION_RELAY_LANGUAGE: str = Field("en-US", env="CONVERSATION_RELAY_LANGUAGE")
    CONVERSATION_RELAY_STT_PROVIDER: str = Field("Google", env="CONVERSATION_RELAY_STT_PROVIDER")
    CONVERSATION_RELAY_SPEECH_MODEL: str = Field("telephony", env="CONVERSATION_RELAY_SPEECH_MODEL")
    CONVERSATION_RELAY_INTERRUPTIBLE: str = Field("any", env="CONVERSATION_RELAY_INTERRUPTIBLE")
    CONVERSATION_RELAY_DTMF_DETECTION: bool = Field(False, env="CONVERSATION_RELAY_DTMF_DETECTION")
