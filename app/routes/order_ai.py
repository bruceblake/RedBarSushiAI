...
def start_order():
    ...
    try:
        ...
        order_state = menu_matcher.interactive_order_resolution(
            customer_request, context
        )

        # Add a conversation field to track the full dialog
        order_state["conversation"] = [{"role": "user", "content": customer_request}]
...
def process_response(session_id):
    ...
    try:
        ...
        updated_state = menu_matcher.process_customer_response(
            order_state, customer_response
        )
