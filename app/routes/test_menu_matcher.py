...
def _manual_test_menu_matcher():
    ...
    try:
        ...
        _manual_test_ai_match(
            app, find_menu_item_ai, "cheeseburger", "I want a burger with cheese"
        )
        _manual_test_ai_match(
            app, find_menu_item_ai, "french fries", "I'd like some fries"
        )
        _manual_test_ai_match(app, find_menu_item_ai, "coke", "I want a soda")

        # Test interactive order resolution with real menu items
        _manual_test_interactive_resolution(
            app, menu_matcher, "I want a burger with fries and a coke"
        )
...
def _manual_test_exact_match(app, menu_matcher, item_name):
    ...
    with app.app_context():
        ...
        if item:
            print(
                f"✅ Found exact match: {item.get('name')} (${item.get('price', 0.0):.2f})"
            )
        else:
...
def _manual_test_ai_match(app, find_menu_item_ai, item_name, context_text=None):
    ...
    with app.app_context():
        ...
        if item:
            print(
                f"✅ Found AI match: {item.get('name')} (${item.get('price', 0.0):.2f})"
            )
        else:
        ...
        for i, response in enumerate(responses):
            ...
            if order_state.get("resolved", False):
                ...
                for item in order_state.get("items", []):
                    print(
                        f"- {item.get('quantity', 1)}x {item.get('name')} (${item.get('price', 0.0):.2f})"
                    )
                break
