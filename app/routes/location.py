...
def product_update_per_location(location_id):
    ...
    try:
        ...
        if success:
            log_info(f"Product {product_id} updated successfully for location {location_id}")
            return jsonify({"success": True}), 200
        else:
...
def modifier_group_update_per_location(location_id):
    ...
    try:
        ...
        if success:
            log_info(f"Modifier group {group_id} updated successfully for location {location_id}")
            return jsonify({"success": True}), 200
        else:
...
def modifier_update_per_location(location_id):
    ...
    try:
        ...
        if success:
            log_info(f"Modifier {modifier_id} updated successfully for location {location_id}")
            return jsonify({"success": True}), 200
        else:
...
def take_order_per_location(location_id):
    ...
    for item_entity in analysis['menu_items']:
        ...
        if "childProducts" in matched_item:
            ...
            if not is_valid:
                response.say(f"Sorry, there's an issue with your order: {error_message}")
                response.hangup()
                return Response(str(response), mimetype='text/xml')
...
def confirm_order_from_initial_per_location(location_id):
    ...
    log_info(f"Order confirmation at {location_id}: Speech='{user_resp}', DTMF='{dtmf_input}'")
    interpreted = None
    if dtmf_input:
    ...
    if interpreted == "yes":
        if len(order_items) == 0:
            response.say(
                "I'm sorry, your order appears to be empty. Please try again.")
            response.hangup()
            return Response(str(response), mimetype='text/xml')
        ...
        try:
            ...
            if response_deliv.status_code != 200:
                log_info(f"Deliverect API error for location {location_id}: Status {response_deliv.status_code}, Response: {response_deliv.text}")
            else:
                log_info(f"Deliverect order for location {location_id} successfully submitted: {response_deliv.text}")
        except requests.RequestException as e:
            log_info(f"Error sending order to Deliverect for location {location_id}: {str(e)}")
