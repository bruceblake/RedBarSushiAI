...
def take_order():
    if BUSY_MODE_ACTIVE:
        ...
        response.say("We're currently busy and not accepting new orders right now. Goodbye!")
        response.hangup()
    ...
    if not available_items:
        response.say("I'm sorry, our menu is currently unavailable. Please try again later.")
        response.hangup()
        return Response(str(response), mimetype='text/xml')
    ...
    if intent != 'order_food' or not analysis.get('menu_items'):
        log_info(f"order_food: {intent}, menu_items: {analysis.get('menu_items')}")
    ...
    with response.gather(
        ...
        g.say(session['order_message'] + " If correct, say yes or press 1. If you need changes, say no or press 2.")
    return Response(str(response), mimetype='text/xml')
...
def confirm_order_from_initial():
    ...
    if interpreted == "yes":
        if not can_process_action(sender, 'order_food', 60):
            response.say("You're placing orders too quickly. Please wait a moment and try again.")
            return Response(str(response), mimetype='text/xml')
        try:
            ...
            db.session.rollback()
            response.say("Sorry, we encountered a database issue. Please try again.")
            return Response(str(response), mimetype='text/xml')
        # Enqueue the confirmation SMS task
        from tasks import send_confirmation_sms_task
        send_confirmation_sms_task.delay(order_id, session.get('order_message', ''), sender, caller_name, session.get('bill_amount', 0), order_items)
        time_taken = 20 + (1 * len(order_items))
        response.say(f"Great! Your order is confirmed and will be ready in about {time_taken} minutes. A confirmation text will be sent. Thank you!")
        response.hangup()
    elif interpreted == "no":
...
def handle_newly_snoozed_in_checkout():
    ...
    if interpreted == "remove":
        filtered_items = [it for it in order_items if not is_item_snoozed_timebased(it)]
        removed_names = [it["name"] for it in order_items if is_item_snoozed_timebased(it)]
        ...
        if not filtered_items:
            with response.gather(
                ...
                g.say("All items removed. Press 1 to order again, 2 for menu, 3 for a real person.")
            return Response(str(response), mimetype='text/xml')
        else:
...
def confirm_order_after_modification():
    ...
    if interpreted == "yes":
        newly_snoozed = [it["name"] for it in order_items if is_item_snoozed_timebased(it)]
        if newly_snoozed:
            with response.gather(
                input='speech dtmf',
                action='/handle_newly_snoozed_in_checkout',
                ...
                g.say("Sorry, the following item(s) are now unavailable: " + ", ".join(newly_snoozed) + ". Press 1 to remove them, 2 to cancel.")
            return Response(str(response), mimetype='text/xml')
        if not can_process_action(sender, 'order_food', 60):
            response.say("You're placing orders too quickly. Please wait and try again.")
            return Response(str(response), mimetype='text/xml')
        try:
            text_msg = session.get('order_message', '')
            new_order = Order(
        ...
        send_confirmation_sms_task.delay(order_id, session.get('order_message', ''), sender, caller_name, session.get('bill_amount', 0), order_items)
        time_taken = 20 + (1 * len(order_items))
        response.say(f"Great! Your order is confirmed and will be ready in about {time_taken} minutes. A confirmation text is on the way. Thank you for choosing Red Bar Sushi! Goodbye.")
        response.hangup()
    elif interpreted == "no":
        with response.gather(
            input='speech',
            ...
            g.say("What else would you like to change? Please describe the final order you want.")
    else:
        with response.gather(
            input='speech dtmf',
            action='/confirm_order_after_modification',
            ...
            g.say("I didn't catch that. Say 'yes' or press 1 if correct, 'no' or press 2 to modify again.")
    return Response(str(response), mimetype='text/xml')
