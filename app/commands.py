...
def init_menu_db():
    ...
    try:
        ...
        if menu_db_store.store_menu_data(file_menu_data):
            click.echo(
                f"Successfully initialized menu database with {len(file_menu_data.get('items', []))} items."
            )
        else:
...
def export_menu(output_file):
    ...
    try:
        ...
        click.echo(
            f"Successfully exported {len(menu_data.get('items', []))} menu items to {output_file}."
        )
