    ...
    try:
        with app.test_client() as client:
            ...
            if command.lower() in ['status', 'stat', 'order', 'check']:
                with app.app_context():
                    existing = db.session.query(Order).filter_by(sender=phone_number).first()
                    if not existing:
                        test_order = Order(
...
Your total is $15.98""",
                        ...
                        db.session.add(test_order)
                        db.session.commit()
                        logger.info(f"Created test order for status check: {test_order.id}")
            ...
            if response.status_code == 200:
                ...
                message_body = message_element.text if message_element is not None and message_element.text else "No message body"
                
                return True, message_body, response_text
            else:
                return False, f"Error: HTTP {response.status_code}", response.data.decode('utf-8')
        ...
        with app.test_client() as client:
            ...
            if response.status_code == 200:
                ...
                if db_status.get('connection') == 'ok':
                    ...
                    print(f"  Registered locations: {db_status.get('registered_locations', 0)}")
                else:
