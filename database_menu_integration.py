...
def update_configuration(skip_config=False):
    ...
    try:
        ...
        if not os.path.exists(menu_utils_db_path):
            logger.error(
                f"Database-backed menu_utils_db.py not found at {menu_utils_db_path}"
            )
            return False
        ...
        if not backup_path:
            logger.warning(
                "Failed to create backup, but continuing with configuration update"
            )
        ...
        if os.name == "posix":  # Unix/Linux/Mac
            try:
                ...
                logger.info(
                    f"Created symlink from {menu_utils_db_path} to {menu_utils_path}"
                )
            except Exception as e:
                ...
                logger.info(
                    f"Copied {menu_utils_db_path} to {menu_utils_path} as fallback"
                )
        else:  # Windows
...
def main():
    ...
    parser.add_argument(
        "--skip-config", action="store_true", help="Skip the configuration update step"
    )
    ...
    logger.info(
        f"Command-line arguments: force={args.force}, location_id={args.location_id}, skip_config={args.skip_config}"
    )
    ...
    try:
        ...
        with app.app_context():
            ...
            from app.utils.menu_migration import (
                migrate_menu_to_database,
                verify_menu_migration,
            )
            ...
            logger.info(
                f"Migrating menu data to database for location_id: {args.location_id or 'default'}..."
            )
            result = migrate_menu_to_database(
                location_id=args.location_id, force=args.force
            )

            if result.get("success"):
                logger.info(
                    f"Migration successful: {result.get('items_count', 0)} items, "
                ...
                if verify_result.get("success"):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to create backup of original menu file: {e}"
                        )

                    # Update configuration to use database
                    if update_configuration(args.skip_config):
                        logger.info(
                            "Application configuration updated to use database-backed implementation"
                        )
                    else:
                        logger.warning("Failed to update application configuration")
                        logger.warning(
                            "You may need to manually update the configuration"
                        )

                    logger.info("Migration and integration completed successfully")
                    logger.info(
                        "The application is now using the PostgreSQL database for menu data"
                    )
                    return 0
                else:
                    ...
                    if verify_result.get("db_items_count", 0) > 0:
                        logger.warning(
                            "Migration verification showed discrepancies, but continuing"
                        )
                        logger.warning(
                            f"File items: {verify_result.get('file_items_count', 0)}, "
                        ...
                        if update_configuration(args.skip_config):
                            ...
                            logger.info(
                                "Migration and integration completed with warnings"
                            )
                            return 0
                        else:
                            logger.warning("Failed to update application configuration")
                        ...
                        logger.error(
                            f"Migration verification failed: {verify_result.get('error', 'Unknown error')}"
                        )
                        logger.error(
                            f"File items: {verify_result.get('file_items_count', 0)}, "
                            f"Database items: {verify_result.get('db_items_count', 0)}"
                ...
                logger.warning(
                    f"Database already contains {result.get('existing_count', 0)} menu items"
                )
