#!/usr/bin/env python3
"""Simple database and service connectivity checker without external dependencies."""
...
def test_db_connection_using_psql(host, user, password, dbname, max_retries=3, retry_delay=2):
    """Test PostgreSQL connectivity using psql command."""
    print(f"Testing database connection to {host}...")
    
    for attempt in range(1, max_retries + 1):
        try:
            # Command to get PostgreSQL version
            cmd = f"PGPASSWORD={password} psql -h {host} -U {user} -d {dbname} -c 'SELECT version();'"
            
            # Execute command
            print(f"Attempt {attempt}/{max_retries}: Executing PostgreSQL query...")
...
def test_redis_connection(host, port, max_retries=3, retry_delay=2):
    """Test Redis connectivity using redis-cli command."""
    print(f"Testing Redis connection to {host}:{port}...")
    
    for attempt in range(1, max_retries + 1):
        try:
            # Command to ping Redis
            cmd = f"redis-cli -h {host} -p {port} ping"
            
            # Execute command
            print(f"Attempt {attempt}/{max_retries}: Pinging Redis...")
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
            ...
            if result.returncode == 0 and "PONG" in result.stdout:
                ...
                print(f"❌ Redis ping failed: {result.stderr.strip() or result.stdout.strip()}")
                
                if attempt < max_retries:
                    print(f"   Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)
        except Exception as e:
            print(f"❌ Error executing Redis ping: {e}")
...
def main():
    ...
    postgres_query_status = False
    if db_url and "@" in db_url:
        try:
            # Extract host, port, user, password, and dbname from DATABASE_URL
            ...
            # Test actual database query if socket connection successful
            if postgres_status:
                postgres_query_status = test_db_connection_using_psql(host, user, password, dbname)
        except Exception as e:
    ...
    if redis_url and "//" in redis_url:
        try:
            ...
            # Test actual Redis query if socket connection successful
            if redis_status:
                redis_query_status = test_redis_connection(host, port)
        except Exception as e:
    ...
    print(f"PostgreSQL socket connection: {'✅' if postgres_status else '❌'}")
    print(f"PostgreSQL query: {'✅' if postgres_query_status else '❌'}")
    print(f"Redis socket connection: {'✅' if redis_status else '❌'}")
    print(f"Redis query: {'✅' if redis_query_status else '❌'}")
    
    # Final status
    if env_status and postgres_status and postgres_query_status and redis_status and redis_query_status:
