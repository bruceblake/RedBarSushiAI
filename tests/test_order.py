        ...
        with patch("app.routes.order.analyze_user_input") as mock_analyze:
            ...
            with patch("app.utils.order_utils.find_menu_item_any_status") as mock_find:
                ...
                response = client.post(
                    "/take_order",
                    data={"SpeechResult": "I would like two California rolls"},
                )
        ...
        with patch("app.routes.order.analyze_user_input") as mock_analyze:
            ...
            with patch("app.utils.order_utils.find_menu_item_any_status") as mock_find:
                ...
                response = client.post(
                    "/take_order",
                    data={
                        "SpeechResult": "I would like a California roll with extra spicy mayo"
                    },
                )
...
def test_busy_mode_toggle(client, app):
    ...
    with patch("app.routes.order.BUSY_MODE_ACTIVE", False) as busy_mode:
        ...
        with app.test_request_context():
            with patch("app.routes.order.BUSY_MODE_ACTIVE", True):
                response = client.post(
                    "/take_order", data={"SpeechResult": "I would like sushi"}
                )
                assert response.status_code == 200
        ...
        with app.test_request_context():
            with patch("app.routes.order.BUSY_MODE_ACTIVE", False):
                with patch("app.routes.order.analyze_user_input") as mock_analyze:
                    ...
                    with patch(
                        "app.utils.order_utils.find_menu_item_any_status"
                    ) as mock_find:
                        mock_find.return_value = (
                            {
                                "name": "California Roll",
                        ...
                        assert (
                            len(response.data) > 0
                        )  # Just check there's some response
...
def test_take_order_unrecognized_intent(client, app, mock_openai, setup_test_menu):
    ...
    with app.test_request_context():
        ...
        with patch("app.routes.order.analyze_user_input") as mock_analyze:
            ...
            response = client.post(
                "/take_order", data={"SpeechResult": "What items do you have?"}
            )
...
def test_take_order_item_not_on_menu(client, app, mock_openai, setup_test_menu):
    ...
    with app.test_request_context():
        ...
        with patch("app.routes.order.analyze_user_input") as mock_analyze:
            mock_analyze.return_value = {
                "intent": "order_food",
                "menu_items": [{"name": "Nonexistent Roll", "quantity": 1}],
            }
            ...
            with patch(
                ...
                response = client.post(
                    "/take_order",
                    data={"SpeechResult": "I would like a Nonexistent Roll"},
                )
...
def test_take_order_unavailable_item(client, app, mock_openai, setup_test_menu):
    ...
    with app.test_request_context():
        ...
        with patch("app.routes.order.analyze_user_input") as mock_analyze:
            mock_analyze.return_value = {
                "intent": "order_food",
                "menu_items": [{"name": "Dragon Roll", "quantity": 1}],
            }
            ...
            with patch(
                "app.utils.order_utils.find_menu_item_any_status",
                return_value=(unavailable_item, 0),
            ):
                response = client.post(
                    "/take_order", data={"SpeechResult": "I would like a Dragon Roll"}
                )
    ...
    with app.test_request_context():
        ...
        with (
            ...
            response = client.post(
                ...
                data={"SpeechResult": "Yes, that is correct"},
            )
...
def test_confirm_order_from_initial_no(client, app):
    ...
    with app.test_request_context():
        ...
        response = client.post(
            "/confirm_order_from_initial",
            data={"SpeechResult": "No, I need to make changes"},
        )
    ...
    with app.test_request_context():
        ...
        with patch("app.routes.order.get_order_modifications") as mock_get_mods:
            ...
            with patch("app.utils.order_utils.find_menu_item") as mock_find:
                ...
                response = client.post(
                    "/new_modify_order", data={"SpeechResult": "Add a spicy tuna roll"}
                )
                ...
                with client.session_transaction() as session:
                    ...
                    assert any(
                        item["name"] == "Spicy Tuna Roll" for item in order_items
                    )
...
def test_get_order_modifications(client, app, mock_openai):
    ...
    with patch("app.utils.agent_utils_simple.get_order_modifications") as mock_get_mods:
        ...
        from app.utils.agent_utils_simple import (
            get_order_modifications as actual_get_order_mods,
        )
...
def test_confirm_order_after_modification_yes(client, app, mock_twilio):
    ...
    with app.test_request_context():
        ...
        with (
            ...
            with client.session_transaction() as session:
                assert (
                    "modification_in_progress" not in session
                    or not session["modification_in_progress"]
                )
...
def test_confirm_order_after_modification_no(client, app):
    ...
    with app.test_request_context():
        ...
        response = client.post(
            "/confirm_order_after_modification",
            data={"SpeechResult": "No, I need to make more changes"},
        )
...
def test_handle_newly_snoozed_in_checkout(client, app, setup_test_menu):
    ...
    with app.test_request_context():
        ...
        with patch("app.utils.menu_utils.load_menu_data") as mock_load:
            ...
            with patch(
                "app.utils.menu_utils.is_item_snoozed_timebased"
            ) as mock_is_snoozed:
                ...
                response = client.post(
                    "/handle_newly_snoozed_in_checkout", data={"SpeechResult": ""}
                )
