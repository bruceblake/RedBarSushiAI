"""
Unit tests for AsyncCartAgent class.

This module tests the cart agent functionality including item lookup,
cart management, and order building capabilities.
"""

import pytest
import asyncio
from unittest.mock import Mock, AsyncMock, patch, MagicMock
from typing import Dict, Any, List

from app.agents.cart_async import AsyncCartAgent
from app.models.menu_async import MenuItem


@pytest.fixture
def mock_db_session():
    """Create a mock database session."""
    session = MagicMock()
    session.commit = AsyncMock()
    session.rollback = AsyncMock()
    session.close = AsyncMock()
    return session


@pytest.fixture
def mock_menu_matcher():
    """Create a mock menu matcher."""
    with patch('app.agents.cart_async.get_cached_async_menu_matcher') as mock_get_matcher:
        matcher = MagicMock()
        
        # Mock successful item match
        async def mock_match_item(item_name, context=None):
            if "california roll" in item_name.lower():
                return {
                    "plu": "CALI_001",
                    "name": "California Roll",
                    "price": 12.95,
                    "description": "Crab, avocado, cucumber",
                    "category": "Rolls"
                }
            elif "spicy tuna" in item_name.lower():
                return {
                    "plu": "TUNA_001",
                    "name": "Spicy Tuna Roll",
                    "price": 13.95,
                    "description": "Spicy tuna, cucumber",
                    "category": "Rolls"
                }
            return None
        
        matcher.match_item = mock_match_item
        mock_get_matcher.return_value = matcher
        yield matcher


@pytest.fixture
def mock_conversation_store():
    """Create a mock conversation store."""
    with patch('app.agents.cart_async.async_agents_conversation_store') as mock_store:
        # Mock cart storage
        cart_data = {"items": []}
        
        async def mock_get_cart(session_id):
            return cart_data.copy()
        
        async def mock_save_cart(session_id, cart):
            cart_data.clear()
            cart_data.update(cart)
        
        async def mock_get_conversation(session_id):
            return {
                "items": cart_data.get("items", []),
                "order_type": "delivery",
                "restaurant": {
                    "name": "Test Restaurant",
                    "id": "test_123"
                }
            }
        
        mock_store.get_cart = mock_get_cart
        mock_store.save_cart = mock_save_cart
        mock_store.get_conversation = mock_get_conversation
        
        yield mock_store


@pytest.fixture
def cart_agent(mock_db_session, mock_menu_matcher, mock_conversation_store):
    """Create a cart agent instance for testing."""
    agent = AsyncCartAgent(agent_id="test_cart_123", db=mock_db_session)
    # Add call_sid to context to avoid errors
    agent.context = {"call_sid": "test_call_123", "session_id": "test_session"}
    return agent


class TestAsyncCartAgent:
    """Test suite for AsyncCartAgent class."""
    
    def test_initialization(self, mock_db_session):
        """Test cart agent initialization."""
        agent = AsyncCartAgent(agent_id="custom_cart", db=mock_db_session)
        
        assert agent.name == "Cart"
        assert agent.agent_id == "custom_cart"
        assert agent.db == mock_db_session
    
    @pytest.mark.asyncio
    async def test_execute_tool_lookup_menu_item(self, cart_agent):
        """Test menu item lookup tool."""
        result = await cart_agent.execute_tool(
            "lookup_menu_item",
            {"item_name": "California Roll"}
        )
        
        assert result["status"] == "success"
        assert result["result"]["plu"] == "CALI_001"
        assert result["result"]["name"] == "California Roll"
        assert result["result"]["price"] == 12.95
    
    @pytest.mark.asyncio
    async def test_execute_tool_lookup_menu_item_not_found(self, cart_agent):
        """Test menu item lookup when item is not found."""
        result = await cart_agent.execute_tool(
            "lookup_menu_item",
            {"item_name": "Unknown Item"}
        )
        
        assert result["status"] == "error"
        assert "not found" in result["result"].lower()
    
    @pytest.mark.asyncio
    async def test_execute_tool_add_item_to_cart(self, cart_agent, mock_conversation_store):
        """Test adding an item to the cart."""
        cart_agent.context["session_id"] = "test_session"
        
        result = await cart_agent.execute_tool(
            "add_item_to_cart",
            {"plu": "CALI_001", "quantity": 2}
        )
        
        assert result["status"] == "success"
        assert "added" in result["result"].lower()
        
        # Verify cart was updated
        cart = await mock_conversation_store.get_cart("test_session")
        assert len(cart["items"]) == 1
        assert cart["items"][0]["plu"] == "CALI_001"
        assert cart["items"][0]["quantity"] == 2
    
    @pytest.mark.asyncio
    async def test_execute_tool_remove_from_cart(self, cart_agent, mock_conversation_store):
        """Test removing an item from the cart."""
        cart_agent.context["session_id"] = "test_session"
        
        # First add an item
        await cart_agent.execute_tool(
            "add_item_to_cart",
            {"plu": "CALI_001", "quantity": 1}
        )
        
        # Then remove it
        result = await cart_agent.execute_tool(
            "remove_from_cart",
            {"item_index": 0}
        )
        
        assert result["status"] == "success"
        assert "removed" in result["result"].lower()
        
        # Verify cart is empty
        cart = await mock_conversation_store.get_cart("test_session")
        assert len(cart["items"]) == 0
    
    @pytest.mark.asyncio
    async def test_execute_tool_modify_cart_item(self, cart_agent, mock_conversation_store):
        """Test modifying a cart item."""
        cart_agent.context["session_id"] = "test_session"
        
        # First add an item
        await cart_agent.execute_tool(
            "add_item_to_cart",
            {"plu": "CALI_001", "quantity": 1}
        )
        
        # Then modify it
        result = await cart_agent.execute_tool(
            "modify_cart_item",
            {"item_index": 0, "new_quantity": 3}
        )
        
        assert result["status"] == "success"
        assert "updated" in result["result"].lower()
        
        # Verify quantity was updated
        cart = await mock_conversation_store.get_cart("test_session")
        assert cart["items"][0]["quantity"] == 3
    
    @pytest.mark.asyncio
    async def test_process_input_add_item_request(self, cart_agent):
        """Test processing natural language add item request."""
        with patch.object(cart_agent, '_process_with_ai', new_callable=AsyncMock) as mock_ai:
            mock_ai.return_value = {
                "text": "I'll add 2 California Rolls to your cart.",
                "needs_action": True,
                "action": {
                    "type": "add_item",
                    "item_name": "California Roll",
                    "quantity": 2
                }
            }
            
            response = await cart_agent.process_input(
                "I want two California rolls please",
                {"session_id": "test_session"}
            )
            
            assert response["handled"] is True
            assert "california roll" in response["text"].lower()
            assert response["agent"] == "Cart"
    
    def test_cart_summary_generation(self, cart_agent):
        """Test cart summary generation."""
        cart = {
            "items": [
                {
                    "plu": "CALI_001",
                    "name": "California Roll",
                    "price": 12.95,
                    "quantity": 2,
                    "modifiers": []
                },
                {
                    "plu": "TUNA_001",
                    "name": "Spicy Tuna Roll",
                    "price": 13.95,
                    "quantity": 1,
                    "modifiers": []
                }
            ],
            "subtotal": 39.85,
            "tax": 3.99,
            "total": 43.84
        }
        
        summary = cart_agent._generate_cart_summary(cart)
        
        assert "California Roll" in summary
        assert "Spicy Tuna Roll" in summary
        assert "39.85" in summary or "39.9" in summary
        assert "43.84" in summary or "43.8" in summary
    
    @pytest.mark.asyncio
    async def test_invalid_tool_parameters(self, cart_agent):
        """Test handling of invalid tool parameters."""
        # Missing required parameter
        result = await cart_agent.execute_tool(
            "add_item_to_cart",
            {"quantity": 2}  # Missing plu
        )
        
        assert result["status"] == "error"
        assert "missing" in result["result"].lower() or "required" in result["result"].lower()
    
    @pytest.mark.asyncio
    async def test_cart_persistence(self, cart_agent, mock_conversation_store):
        """Test that cart changes are persisted."""
        cart_agent.context["session_id"] = "test_session"
        
        # Add multiple items
        await cart_agent.execute_tool(
            "add_item_to_cart",
            {"plu": "CALI_001", "quantity": 1}
        )
        
        await cart_agent.execute_tool(
            "add_item_to_cart",
            {"plu": "TUNA_001", "quantity": 2}
        )
        
        # Verify cart has both items
        cart = await mock_conversation_store.get_cart("test_session")
        assert len(cart["items"]) == 2
        assert cart["items"][0]["plu"] == "CALI_001"
        assert cart["items"][1]["plu"] == "TUNA_001"


@pytest.fixture
def cart_agent_with_errors(mock_db_session, mock_menu_matcher, mock_conversation_store):
    """Create a cart agent for error testing."""
    agent = AsyncCartAgent(agent_id="error_cart", db=mock_db_session)
    agent.context = {"call_sid": "test_call_123"}
    return agent


class TestCartAgentEdgeCases:
    """Test edge cases for cart agent."""
    
    @pytest.mark.asyncio
    async def test_remove_from_empty_cart(self, cart_agent_with_errors):
        """Test removing from an empty cart."""
        cart_agent_with_errors.context["session_id"] = "test_session"
        
        with patch('app.agents.cart_async.async_agents_conversation_store.get_cart') as mock_get:
            mock_get.return_value = {"items": []}
            
            result = await cart_agent_with_errors.execute_tool(
                "remove_from_cart",
                {"item_index": 0}
            )
            
            assert result["status"] == "error"
            assert "empty" in result["result"].lower() or "no items" in result["result"].lower()
    
    @pytest.mark.asyncio
    async def test_invalid_item_index(self, cart_agent_with_errors):
        """Test modifying with invalid item index."""
        cart_agent_with_errors.context["session_id"] = "test_session"
        
        with patch('app.agents.cart_async.async_agents_conversation_store.get_cart') as mock_get:
            mock_get.return_value = {"items": [{"plu": "TEST", "quantity": 1}]}
            
            result = await cart_agent_with_errors.execute_tool(
                "modify_cart_item",
                {"item_index": 10, "new_quantity": 2}  # Invalid index
            )
            
            assert result["status"] == "error"
            assert "invalid" in result["result"].lower() or "index" in result["result"].lower()
    
    @pytest.mark.asyncio
    async def test_concurrent_cart_modifications(self, cart_agent_with_errors):
        """Test handling concurrent cart modifications."""
        cart_agent_with_errors.context["session_id"] = "test_session"
        
        async def add_item(plu, quantity):
            return await cart_agent_with_errors.execute_tool(
                "add_item_to_cart",
                {"plu": plu, "quantity": quantity}
            )
        
        # Simulate concurrent additions
        results = await asyncio.gather(
            add_item("ITEM1", 1),
            add_item("ITEM2", 2),
            add_item("ITEM3", 3),
            return_exceptions=True
        )
        
        # All operations should complete without exceptions
        for result in results:
            assert not isinstance(result, Exception)
            if isinstance(result, dict):
                assert result.get("status") in ["success", "error"]