...
class TestDatabaseRetryLogic:
    ...
    def test_execute_with_retry_success_after_failures(self):
        ...
        with patch("app.db_init.time.sleep"), patch(
            "app.db_init.verify_connection", return_value=True
    ...
    def test_execute_with_retry_backoff_timing(self):
        ...
        with patch.dict(
            ...
            with patch("app.db_init.time.sleep", mock_sleep), patch(
                "app.db_init.random.uniform", return_value=0.1
            ), patch("app.db_init.verify_connection", return_value=True), patch(
                "app.db_init.db.session"
            ):
                result = execute_with_retry(mock_func)
    ...
    def test_execute_with_retry_non_db_error(self):
        ...
        with patch("app.db_init.time.sleep"), patch(
            "app.db_init.verify_connection", return_value=True
        ), patch("app.db_init.db.session"), pytest.raises(KeyError):
