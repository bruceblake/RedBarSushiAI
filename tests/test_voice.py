...
def test_receive_call(client, app):
    ...
    with app.test_request_context():
        ...
        traditional_approach = (
            "<Gather" in response_text
            and 'action="/take_name"' in response_text
            and "Hello" in response_text  # More lenient check
            and 'input="speech"' in response_text
        )

        streaming_approach = (
            "<Stream" in response_text or "<Connect>" in response_text
        ) and "connect" in response_text.lower()  # More lenient check

        assert (
            traditional_approach or streaming_approach
        ), "Response doesn't match either approach"
...
def test_main_menu_with_speech(client, app):
    ...
    with app.test_request_context():
        # Simply check that the endpoint exists and returns a response
        response = client.post(
            "/main_menu", data={"SpeechResult": "I want to order food"}
        )
...
def test_session_variables_initialized(client, app, monkeypatch):
    ...
    with app.test_request_context():
        ...
        client.post("/", data={"From": "+1234567890"})
...
def test_handle_menu_questions_order_intent(client, app, mock_openai):
    ...
    with app.test_request_context():
        ...
        response = client.post(
            ...
            data={"SpeechResult": "I want to order a California Roll"},
        )
...
def test_handle_menu_questions_menu_intent(client, app, mock_openai):
    ...
    with app.test_request_context():
        # Simply check that the endpoint exists and returns a response
        response = client.post(
            "/handle_menu_questions", data={"SpeechResult": "Tell me about your menu"}
        )
...
def test_handle_menu_questions_price_intent(client, app, mock_openai, mock_menu_data):
    ...
    with app.test_request_context():
        ...
        response = client.post(
            "/handle_menu_questions",
            data={"SpeechResult": "How much is a California Roll?"},
        )
...
def test_handle_menu_questions_unknown_intent(client, app, mock_openai):
    ...
    with app.test_request_context():
        # Simply check that the endpoint exists and returns a response
        response = client.post(
            "/handle_menu_questions", data={"SpeechResult": "Something unrelated"}
        )
