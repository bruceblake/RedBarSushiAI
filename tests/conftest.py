...
def mock_openai(monkeypatch):
    ...
    class MockCompletions:
        def create(self, *args, **kwargs):
            ...
            if any(
                "modification request" in msg.get("content", "")
                for msg in messages
        ...
        def create_thread(self):
            ...
            mock_message = type(
                ...
                (object,),
                {
                    "id": "msg_123",
                    "content": [
                        type("obj", (object,), {"text": type("obj", (object,), {})})
                    ],
                },
            )
            ...
            mock_runs = type(
                "obj",
                (object,),
                {"create": lambda: mock_run, "wait": lambda run_id: mock_run},
            )
    ...
    monkeypatch.setattr(
        "app.utils.agent_utils_simple.analyze_user_input", mock_analyze_user_input
    )
    monkeypatch.setattr(
        "app.utils.agent_utils_simple.get_order_modifications",
        mock_get_order_modifications,
    )
...
def mock_deliverect(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.utils.deliverect.get_deliverect_token", mock_get_deliverect_token
    )
...
def setup_test_menu(app):
    ...
    test_menu_path = os.path.join(
        os.path.dirname(__file__), "..", "test_menu_data.json"
    )
    ...
    os.makedirs(
        os.path.join(os.path.dirname(__file__), "..", "testing_data"), exist_ok=True
    )
...
def mock_menu_data():
    ...
    return {
        ...
        "modifiers": [
            {"name": "Extra Wasabi", "price": 0.50, "reference_handler": "mod-wasabi-1"}
        ],
        "modifierGroups": [{"name": "Additions", "modifiers": ["mod-wasabi-1"]}],
        "name_variants": {
            "california roll": "California Roll",
            "spicy tuna roll": "Spicy Tuna Roll",
        },
