            ...
            try:
                ...
                if say_element is not None and say_element.text:
                    ...
                    if "STAGING" in say_element.text:
                        ...
                        print(
                            "⚠️ UNKNOWN: Could not determine environment from voice message"
                        )
                        return None
                else:
...
def print_problem_solving_guidance(results):
    ...
    if all(env == "staging" for env in results.values() if env is not None):
        print(
            "✅ SUCCESS! All endpoints are correctly identifying as the STAGING environment."
        )
        ...
        print(
            "   - Ensure FLASK_ENV=staging is set in your Render environment variables"
        )
        ...
        print(
            "   - Check if the Render service is correctly deploying from the staging branch"
        )
    ...
    print("1. Check the logs in your staging environment")
    print(f"2. Try visiting {args.url}/environment in your browser")
    print(
        "3. Verify that your Render service is correctly configured to deploy from the 'staging' branch"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test Twilio webhook configuration with staging environment"
    )
    parser.add_argument(
        "--url",
        required=True,
        help="Base URL of the staging environment (e.g., https://redbarsushi-staging.onrender.com)",
    )
