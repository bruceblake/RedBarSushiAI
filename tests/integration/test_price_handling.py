...
def test_price_consistency(flask_client):
    ...
    if 10 <= dollar_price <= 20:  # Dollar format
        ...
        pytest.fail(f"Unexpected price format. Dollar item: {dollar_price}, Cent item: {cent_price}")
...
def test_price_update_consistency(flask_client):
    ...
    updated_item = next((i for i in menu_data['items'] if i['plu'] == 'UPDATE-01'), None)
    ...
    # Find the updated item
    dollar_updated_item = next((i for i in dollar_menu_data['items'] if i['plu'] == 'UPDATE-01'), None)
