...
def test_deliverect_price_conversion(flask_client, deliverect_menu_json):
    ...
    flask_client.post(
        '/menu_update',
        data=json.dumps(deliverect_menu_json),  # Assumes this fixture has prices in cents
    ...
    updated_item = next((i for i in updated_menu_data['items'] 
                        if i['plu'] == sample_item['plu']), None)
    
    assert updated_item is not None, f"Could not find item with PLU {sample_item['plu']}"
...
def test_price_rounding_consistency(flask_client):
    ...
    updated_item1 = next((i for i in updated_data['items'] if i['plu'] == 'TEST-PRICE-01'), None)
    updated_item2 = next((i for i in updated_data['items'] if i['plu'] == 'TEST-PRICE-02'), None)
    updated_item3 = next((i for i in updated_data['items'] if i['plu'] == 'TEST-PRICE-03'), None)
    
    # Check if prices match expected values
    if updated_item1['price'] >= 100:  # If price is in cents
        assert round(updated_item1['price']) == 999, \
            f"Updated price for item 1 incorrect. Expected 999, got {updated_item1['price']}"
        assert round(updated_item2['price']) == 1049, \
            f"Updated price for item 2 incorrect. Expected 1049, got {updated_item2['price']}"
        assert round(updated_item3['price']) == 795, \
            f"Updated price for item 3 incorrect. Expected 795, got {updated_item3['price']}"
    else:  # If price is in dollars
        assert abs(updated_item1['price'] - 9.99) < 0.01, \
            f"Updated price for item 1 incorrect. Expected 9.99, got {updated_item1['price']}"
        assert abs(updated_item2['price'] - 10.49) < 0.01, \
            f"Updated price for item 2 incorrect. Expected 10.49, got {updated_item2['price']}"
        assert abs(updated_item3['price'] - 7.95) < 0.01, \
...
def test_mixed_price_format_handling(flask_client):
    ...
    # Get the items
    dollar_item = next((i for i in menu_data['items'] if i['plu'] == 'PRICE-FMT-01'), None)
    cent_item = next((i for i in menu_data['items'] if i['plu'] == 'PRICE-FMT-02'), None)
    ...
    if dollar_item['price'] >= 100:
        ...
        assert abs(dollar_item['price'] - dollar_expected_cents) < 1, \
            f"Dollar-format price not converted correctly. Expected ~{dollar_expected_cents}, got {dollar_item['price']}"
        assert abs(cent_item['price'] - cent_expected_cents) < 1, \
    ...
    # If the API returns prices in dollars
        ...
        assert abs(dollar_item['price'] - dollar_expected) < 0.01, \
            f"Dollar-format price not preserved. Expected {dollar_expected}, got {dollar_item['price']}"
        assert abs(cent_item['price'] - cent_expected) < 0.01, \
