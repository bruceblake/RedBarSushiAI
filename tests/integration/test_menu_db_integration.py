...
def test_migrate_menu_to_database(app_with_db, test_menu_data):
    ...
    with app_with_db.app_context():
        ...
        try:
            ...
            assert result["modifier_groups_count"] == len(
                test_menu_data["modifierGroups"]
            )
...
def test_load_menu_data(app_with_db, test_menu_data):
    ...
    with app_with_db.app_context():
        ...
        assert len(loaded_data["modifierGroups"]) == len(
            test_menu_data["modifierGroups"]
        )
...
def test_update_menu_item(app_with_db, test_menu_data):
    ...
    with app_with_db.app_context():
        ...
        item = MenuItem.query.filter_by(
            reference_handler=test_item["reference_handler"]
        ).first()
        ...
        updated_item = next(
            (
                ...
                if item["reference_handler"] == test_item["reference_handler"]
            ),
            None,
        )
    ...
    with app_with_db.app_context():
        ...
        updated_item = next(
            (
                item
                for item in loaded_data["items"]
                if item["reference_handler"] == product_id
            ),
            None,
        )
