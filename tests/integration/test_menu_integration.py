...
@pytest.mark.integration
def test_menu_update_integration_with_deliverect_api(
    app, flask_client, mock_deliverect
):
    """
    ...
    response = flask_client.post(
        "/menu_update", data=json.dumps(async_menu), content_type="application/json"
    )
    ...
    assert "https://api.deliverect.com/testcallback" in str(
        args
    ) or "https://api.deliverect.com/testcallback" in str(kwargs)
...
def test_snooze_integration_with_menu_update(flask_client, simple_menu_format):
    ...
    snooze_response = flask_client.post(
        "/menu_update", data=json.dumps(snooze_payload), content_type="application/json"
    )
    ...
    snoozed_item = next(
        (i for i in updated_menu["items"] if i["plu"] == item_to_snooze["plu"]), None
    )
    ...
    updated_item = next(
        (i for i in post_update_menu["items"] if i["plu"] == item_to_snooze["plu"]),
        None,
    )
    ...
    unsnooze_payload = {
        ...
        "channelLinkId": "test-channel-link-id",
        "operations": [
            {"action": "unsnooze", "data": {"items": [{"plu": item_to_snooze["plu"]}]}}
        ],
        "allSnoozedItems": [],
    }
    ...
    unsnoozed_item = next(
        (i for i in final_menu["items"] if i["plu"] == item_to_snooze["plu"]), None
    )
    assert unsnoozed_item is not None
    assert (
        unsnoozed_item.get("snoozed") is not True
        and unsnoozed_item.get("available") is not False
    )
...
def test_menu_matcher_integration_with_menu_update(flask_client, simple_menu_format):
    ...
    for variation in variations:
        ...
        matcher_payload = {"query": variation, "max_results": 1}
        ...
        if match_response.status_code == 200:
            ...
            assert (
                match_result.get("plu") == item_to_match["plu"]
                or match_result.get("items")[0].get("plu") == item_to_match["plu"]
            )
        else:
    ...
    matcher_payload = {"query": updated_variation, "max_results": 1}

    match_response = flask_client.post(
        "/menu/match", data=json.dumps(matcher_payload), content_type="application/json"
    )
