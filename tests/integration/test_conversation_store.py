...
def test_conversation_store_basic_operations():
    ...
    initial_data = {
        ...
        "messages": [
            {"role": "user", "content": "Tell me about your menu", "timestamp": time.time()}
        ],
        "context": {},
        "resolved": False,
        "items": []
    ...
    # Add a message
    result = conversation_store.add_message(session_id, "assistant", "We have sushi and more!")
    ...
    assert empty["id"] == session_id, "Should return a new empty conversation with the same ID"
...
def test_conversation_context_preservation():
    ...
    conversation_store.add_message(session_id, "user", "Tell me about the Spicy Tuna Roll")
    ...
    assert len(conversation["messages"]) == 4, "Should have 4 messages in the conversation"
    
    # Verify the conversation flow makes sense
    assert "sushi" in conversation["messages"][0]["content"], "First message should be about sushi"
    assert "California" in conversation["messages"][1]["content"], "Second message should mention California Roll"
    assert "Spicy Tuna" in conversation["messages"][2]["content"], "Third message should be about Spicy Tuna"
    assert "$12.99" in conversation["messages"][3]["content"], "Fourth message should include price"
    
    # Verify context is preserved
    assert conversation["context"].get("caller_name") == "Test User", "Context should be preserved"
...
def test_fallback_to_memory_when_redis_unavailable():
    ...
    conversation_store.save_conversation(session_id, {
        ...
        "created_at": time.time(),
        "updated_at": time.time(),
        "messages": [{"role": "user", "content": "Memory test", "timestamp": time.time()}],
        "context": {},
        "resolved": False,
        "items": []
    })
    ...
    assert session_id in conversation_store.memory_store, "Conversation not saved to memory"
    
    # Retrieve and verify the conversation
    conversation = conversation_store.get_conversation(session_id)
    assert conversation["id"] == session_id, "Session ID mismatch"
    assert "Memory test" in conversation["messages"][0]["content"], "Message content mismatch"
    
    # Clean up
    conversation_store.delete_conversation(session_id)
    assert session_id not in conversation_store.memory_store, "Conversation not deleted from memory"