...
def test_menu_update_with_deliverect_format(
    flask_client, deliverect_menu_json, mock_deliverect
):
    """
...
@pytest.mark.integration
def test_menu_update_with_async_format(
    flask_client, deliverect_async_menu_json, mock_deliverect
):
    """
    ...
    mock_deliverect.assert_any_call(callback_url, json=mock.ANY)
...
@pytest.mark.integration
def test_snooze_unsnooze_functionality(
    flask_client,
    deliverect_menu_json,
    deliverect_snooze_payload,
    deliverect_unsnooze_payload,
):
    """
...
def test_partial_menu_update(flask_client, simple_menu_format):
    ...
    response = flask_client.post(
        "/menu_update", data=json.dumps(partial_update), content_type="application/json"
    )
