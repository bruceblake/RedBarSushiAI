...
def test_plu_variant_handling(flask_client):
    ...
    variant_menu = {
        ...
        "data": {
            "categories": [
                {"id": "BURG-CAT", "name": "Burgers", "description": "Burger options"}
            ],
            "items": [
    ...
    response = flask_client.post(
        "/menu_update", data=json.dumps(variant_menu), content_type="application/json"
    )
    ...
    assert (
        items["P-BURG-CHK"]["price"] == 8.99
    ), f"Expected base product price 8.99, got {items['P-BURG-CHK']['price']}"
    assert (
        items["P-BURG-CHE"]["price"] == 9.49
    ), f"Expected base product price 9.49, got {items['P-BURG-CHE']['price']}"
    ...
    assert (
        chicken_variant["price"] > 0
    ), f"Chicken Burger variant has invalid price: {chicken_variant['price']}"
    assert (
        cheese_variant["price"] > 0
    ), f"Cheeseburger variant has invalid price: {cheese_variant['price']}"
    ...
    assert (
        updated_chicken_variant is not None
    ), "Couldn't find updated Chicken Burger variant"
    assert (
        updated_chicken_variant["price"] == 10.99
    ), f"Expected updated price 10.99, got {updated_chicken_variant['price']}"
...
def test_missing_price_handling(flask_client):
    ...
    # Submit base menu
    response = flask_client.post(
        "/menu_update", data=json.dumps(base_menu), content_type="application/json"
    )
    ...
    response2 = flask_client.post(
        "/menu_update", data=json.dumps(extracted_base), content_type="application/json"
    )

    assert (
        response2.status_code == 200
    ), "Failed to add product with extractable base PLU"
    ...
    response3 = flask_client.post(
        "/menu_update", data=json.dumps(missing_price), content_type="application/json"
    )

    assert (
        response3.status_code == 400
    ), "Should reject product with missing price and no reference"
    ...
    assert (
        referenced_product["price"] == 12.99
...
def test_empty_database_handling(flask_client):
    ...
    try:
        clear_resp = flask_client.post(
            "/clear_menu_data", content_type="application/json"
        )
    ...
    response = flask_client.post(
        "/menu_update", data=json.dumps(empty_db_menu), content_type="application/json"
    )

    # Check that the request was rejected
    assert (
        response.status_code == 400
    ), "Should reject item with invalid price when database is empty"

    # Check that error message mentions empty database
    assert (
        "database is empty" in response.data.decode("utf-8").lower()
...
def test_variant_product_handling(flask_client):
    ...
    ), "Failed to accept variant product with zero price"
    ...
    assert (
        parent_variant["price"] == 0
    ), f"Parent variant product should have zero price, got {parent_variant['price']}"
    assert (
        parent_variant.get("is_variant") == True
    ), "Parent product should be marked as variant"
