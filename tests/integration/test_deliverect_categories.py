...
def test_category_preservation(flask_client, deliverect_menu_json):
    ...
    output_categories = {cat.get('id'): cat.get('name') for cat in menu_data['categories']}
    for cat_id, cat_name in input_categories.items():
        assert cat_id in output_categories, f"Category ID {cat_id} missing from output menu"
        assert output_categories[cat_id] == cat_name, \
    ...
    for item in menu_data['items']:
        if 'categoryId' in item:
            assert item['categoryId'] in output_categories, \
...
def test_category_update(flask_client):
    ...
    if dessert_category_exists:
        ...
        assert items['ITEM-3'].get('categoryId') is None or \
               items['ITEM-3'].get('categoryId') != 'CAT-3', \
...
def test_nested_categories(flask_client):
    ...
    assert updated_categories['GRANDCHILD-1'].get('parentId') == 'PARENT-2', \
