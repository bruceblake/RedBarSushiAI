...
def test_reference_handlers_for_deliverect(client):
    ...
    kids_burger = next((item for item in items if item.get("name") == "Kids Burger"), None)
    assert kids_burger is not None
    assert kids_burger.get("reference_handler") != ""
    assert "KidsBurger" in kids_burger.get("reference_handler") or "PROD-" in kids_burger.get("reference_handler")
