    ...
    if len(data.get('items', [])) == 1 and 'products' in data['items'][0]:
        ...
        for product_id, product in products.items():
            ...
            for cat in categories:
                if product_id in cat.get('products', []) or product_id in cat.get('subProducts', []):
        ...
        if success:
            print(f"Successfully extracted and wrote {len(new_menu['items'])} items to menu_data.json")
        else:
