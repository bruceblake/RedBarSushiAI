...
def test_menu_import(monkeypatch):
    ...
    assert (
        "name_variants" in processed
    ), "Processed result should contain 'name_variants'"
    assert (
        len(processed["items"]) > 0
    ), "There should be at least one item in processed result"
    ...
    salmon_nigiri = next(
        (item for item in processed["items"] if item["name"] == "Salmon Nigiri"), None
    )
    ...
    california_roll = next(
        (item for item in processed["items"] if item["name"] == "California Roll"), None
    )
    assert (
        california_roll is not None
    ), "California Roll should be in the processed items"
