...
sys.modules["celery"] = type(
    "celery", (), {"Celery": MockCelery, "Task": type("Task", (), {})}
)
...
def test_register_new_location(app):
    ...
    with app.app_context():
        ...
        result = register_new_location(location_id=location_id, location_name=new_name)
        ...
        "categories": [
            {
                ...
                "name": "Sushi",
                "products": [
                    {"id": "california_roll", "name": "California Roll", "price": 995}
                ],
            }
        ]
    }
...
def test_product_update_per_location(client, app_with_locations):
    ...
    with patch("app.routes.location.process_product_changes") as mock_process:
        ...
        mock_process.assert_called_once_with(
            "california_roll", product_data, "downtown"
        )
...
def test_snooze_unsnooze_per_location(client, app_with_locations):
    ...
    # Sample snooze data
    snooze_data = {
        "operations": [{"item": "California Roll", "action": "snooze", "duration": 60}]
    }
...
def test_take_order_per_location(client, app_with_locations):
    ...
    with patch("app.routes.location.LOCATIONS_BUSY_STATUS", {"downtown": True}):
        response = client.post(
            "/location/downtown/take_order", data={"SpeechResult": "I'd like sushi"}
        )
