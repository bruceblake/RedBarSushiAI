...
def test_update_reference_success_and_not_found(client, monkeypatch):
    # Prepare mock menu
    menu_data = {
        "items": [{"name": "Test Item", "reference_handler": "old", "available": True}],
        "modifierGroups": [],
        "name_variants": {},
    }
    monkeypatch.setattr(
        "app.routes.menu.load_menu_data", lambda force_refresh=True: menu_data
    )
    writes = []
    ...
    # Successful update
    resp1 = client.post(
        "/update_reference", json={"item_name": "Test Item", "reference_handler": "new"}
    )
    assert resp1.status_code == 200
    assert writes and writes[0]["items"][0]["reference_handler"] == "new"
    # Not found
    resp2 = client.post(
        "/update_reference", json={"item_name": "No Such", "reference_handler": "x"}
    )
...
def test_sync_references_success_and_error(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.routes.menu.sync_reference_handlers", lambda **kwargs: {"count": 3}
    )
    resp1 = client.post(
    ...
    monkeypatch.setattr("app.routes.menu.sync_reference_handlers", fail)
    resp2 = client.post(
        "/sync_references", json={"source_location": "A", "target_location": "B"}
    )
...
def test_get_menu_and_clear_cache(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda force_refresh=True, location_id=None: data,
    )
    # GET /menu
