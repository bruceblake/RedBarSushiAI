...
def test_handle_menu_questions_dynamic_success(client, monkeypatch):
    ...
    class FakeResult:
        choices = [type('C', (object,), {'message': type('M', (object,), {'content': fake_reply})})]
    class FakeClient:
        def __init__(self):
            self.chat = type('Chat', (), {})()
            self.chat.completions = type('Comp', (), {'create': lambda *args, **kwargs: FakeResult()})()
    # Ensure OPENAI is enabled
    monkeypatch.setenv('DISABLE_OPENAI', 'false')
    # Monkeypatch OpenAI client class
    monkeypatch.setattr(voice_mod.openai, 'OpenAI', lambda *args, **kwargs: FakeClient())
