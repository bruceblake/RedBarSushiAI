...
def test_list_format_with_string_products():
    ...
    valid_products = [
        item for item in result["items"] if "product" in item.get("name", "").lower()
    ]
...
def test_minimal_product_list():
    ...
    test_data = [
        {"id": "p1", "name": "California Roll", "price": 995, "plu": "CAL-ROLL"},
        {"id": "p2", "name": "Spicy Tuna Roll", "price": 1095, "plu": "SPICY-TUNA"},
    ]
...
def test_mixed_valid_invalid_products():
    ...
    valid_product = next(
        item for item in result["items"] if item["name"] == "Valid Product"
    )
...
def test_name_variants():
    ...
    test_data = [
        {"id": "p1", "name": "Spicy Tuna Roll", "price": 1095, "plu": "SPICY-TUNA"}
    ]
