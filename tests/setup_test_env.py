...
def create_sample_data():
    ...
    menu_data = {
        ...
        "modifierGroups": [
            {"name": "Additions", "modifiers": ["mod-wasabi-1", "mod-ginger-1"]}
        ],
        "name_variants": {
...
def setup_postgresql_test_db():
    ...
    try:
        ...
        print(
            "Failed to create PostgreSQL test database. Make sure PostgreSQL is installed and running."
        )
        return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Set up a testing environment for the restaurant AI agent"
    )
    parser.add_argument(
        "--render-compatible",
        action="store_true",
        help="Create a Render-compatible configuration",
    )
    parser.add_argument(
        "--with-sample-data", action="store_true", help="Create sample data files"
    )
    parser.add_argument(
        "--setup-postgres",
        action="store_true",
        help="Set up a PostgreSQL test database",
    )

    args = parser.parse_args()

    # Create test configuration
    create_test_config(render_compatible=args.render_compatible)
    ...
    if args.with_sample_data:
        ...
        shutil.copy(
            data_paths["menu_path"],
            os.path.join(project_root, "tests", "test_menu_data.json"),
        )
    ...
    print(
        "  python -m pytest tests/test_ai_agent.py                   # Test AI agent functionality"
    )
    print(
        "  python -m pytest tests/simulation/test_customer_interaction.py  # Test customer interactions"
    )
    print(
        "  python -m pytest tests/load/test_concurrent_users.py      # Run load testing"
    )
