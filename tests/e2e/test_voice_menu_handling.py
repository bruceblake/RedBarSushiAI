...
def test_voice_menu_inquiry_handling(api_request, create_test_menu_payload):
    ...
    logger.info(
        f"Test menu contains {len(test_items)} items in {len(categories)} categories"
    )
    for idx, item in enumerate(test_items[:5]):  # Log first 5 items
        logger.info(
            f"Test item {idx+1}: {item['name']} (PLU: {item['plu']}, Price: {item['price']})"
        )
    ...
    take_name_response = api_request.post(
        "/take_name", data={"CallSid": session_id, "Digits": "1"}
    )
    assert take_name_response.status == 200
    logger.info("Voice session initialized with take_name endpoint")

    # Step 4: Confirm the name
    confirm_name_response = api_request.post(
        "/confirm_name", data={"CallSid": session_id, "Digits": "1"}
    )
    ...
    menu_inquiry_response = api_request.post(
        "/handle_menu_questions",
        data={"CallSid": session_id, "SpeechResult": menu_inquiry_text},
    )
    ...
    item_mentioned = any(
        item["name"].lower() in response_text.lower() for item in test_items
    )
    ...
    order_response = api_request.post(
        "/new_order", data={"CallSid": session_id, "SpeechResult": order_text}
    )
    assert order_response.status == 200
    logger.info(f"Placed order for test item: '{test_item['name']}'")

    # Step 7: Confirm the order
    confirm_order_response = api_request.post(
        "/confirm_order", data={"CallSid": session_id, "Digits": "1"}  # 1 for confirm
    )
    ...
    modify_response = api_request.post(
        "/new_modify_order", data={"CallSid": session_id, "SpeechResult": modify_text}
    )
    ...
    assert (
        second_item["name"].lower() in modify_response.text.lower()
    ), f"Response should confirm adding {second_item['name']}"
    logger.info("Modification response includes the added item as expected")

    # Step 9: Complete the order
    complete_response = api_request.post(
        "/complete_order", data={"CallSid": session_id, "Digits": "1"}  # 1 for complete
    )
    ...
    item_update_response = api_request.post(
        "/update_menu_item", data={"item_data": json.dumps(test_items[0])}
    )
    assert item_update_response.status == 200
    logger.info(
        f"Updated price of '{test_items[0]['name']}' from {original_price} to {test_items[0]['price']}"
    )
    ...
    price_inquiry_response = api_request.post(
        "/handle_menu_questions",
        data={"CallSid": session_id, "SpeechResult": new_menu_inquiry},
    )
    ...
    logger.info(
        f"Checking that response shows new price {new_price_str} instead of old price {old_price_str}"
    )

    response_text = price_inquiry_response.text
    assert (
        new_price_str in response_text or str(test_items[0]["price"]) in response_text
    ), "Response should include the updated price"
    assert (
        old_price_str not in response_text
    ), "Response should not contain the old price"
...
def test_menu_matcher_error_recovery(api_request, create_test_menu_payload):
    ...
    session_id = f"test-session-{uuid.uuid4()}"
    logger.info(
        f"Starting menu matcher error recovery test with session ID: {session_id}"
    )
    ...
    invalid_order_response = api_request.post(
        "/new_order",
        data={
            "CallSid": session_id,
    ...
    assert (
        ...
        or "don't" in response_text.lower()
        or "can't" in response_text.lower()
    ), "Response should indicate the item wasn't found"
    ...
    valid_order_response = api_request.post(
        "/new_order",
        data={
            "CallSid": session_id,
    ...
    assert (
        valid_item.lower() in response_text.lower()
    ), f"Response should include the valid item {valid_item}"
