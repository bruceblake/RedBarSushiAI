...
def test_store_closure_impact(api_request, create_test_menu_payload):
    ...
    closure_mentioned = any(
        phrase in greeting_twiml.lower()
        for phrase in [
            "closed",
            "not open",
            "unavailable",
    ...
    if closed_order_response.status != 200:
        assert closed_order_response.status in [
            400,
            403,
            503,
        ...
        if "store_status" in order_result:
            assert (
                order_result["store_status"] == "closed"
                or order_result["store_status"] == "paused"
            )
    ...
    busy_mentioned = any(
        phrase in busy_greeting_twiml.lower()
        for phrase in ["busy", "wait time", "delay", "longer than usual", "30 minute"]
    )
    assert (
        busy_mentioned
    ), "Voice system didn't mention busy status or increased wait time"
    ...
    if "store_status" in order_result:
        assert (
            order_result["store_status"] == "open"
            or order_result["store_status"] == "online"
        )
...
def test_menu_updates_during_active_orders(api_request, create_test_menu_payload):
    ...
    update_response = api_request.post(
        f"/order/{original_order_id}/status", data={"status": "accepted"}
    )
    ...
    assert (
        abs(
            new_order_data["items"][0]["price"]
            - updated_menu_payload["items"][0]["price"]
        )
        < 0.01
    )
...
def test_time_based_availability(api_request, create_test_menu_payload):
    ...
    if future_order_response.status != 200:
        assert future_order_response.status in [
            400,
            409,
            422,
        ], "Unexpected error code for unavailable item"
    else:
        # If order is accepted, response should indicate unavailability
        future_order_result = future_order_response.json()
        assert (
            "unavailable_items" in future_order_result
            or "warnings" in future_order_result
        )
