...
def setup_test_database(app):
    """
    Modified fixture for E2E testing against staging environment.
    This version doesn't attempt to set up a local database but assumes
    we're testing against the remote server.
    """
    # When testing against remote servers, we don't need to set up a local DB
    if os.environ.get("BASE_URL", "").startswith("http"):
        # We're using a remote server, so just yield without setup
        yield
        return
        
    # The rest of this function would only run for local testing
    try:
        ...
        print(f"Skipping database setup due to error: {e}")
        yield
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "use_db_menu: mark a test to use the database for menu operations"
    )
