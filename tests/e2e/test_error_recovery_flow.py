...
def test_speech_recognition_error_recovery(api_request):
    ...
    low_confidence_detected = any(phrase in name_retry_twiml.lower() for phrase in 
                                ["didn't catch", "didn't understand", "could you repeat", "say that again"])
    assert low_confidence_detected, "System didn't handle low confidence recognition properly"
...
def test_order_submission_error_recovery(api_request, create_test_menu_payload):
    ...
    confirm_endpoint = "/order/confirm"  # The actual endpoint that handles final confirmation
    ...
    assert any(phrase in error_twiml.lower() for phrase in 
              ["technical difficulty", "system error", "problem", "issue", "try again"])
    
    # Should offer alternatives (retry, human assistance, etc.)
    recovery_options = any(phrase in error_twiml.lower() for phrase in 
                          ["try again", "speak with", "transfer", "call back"])
...
def test_payment_processing_error_recovery(api_request, create_test_menu_payload):
    ...
    payment_error_detected = any(phrase in error_twiml.lower() for phrase in 
                                ["payment", "card", "declined", "issue", "problem"])
    assert payment_error_detected, "System didn't indicate payment processing error"
    
    # Should offer alternatives
    alternatives_offered = any(phrase in error_twiml.lower() for phrase in 
                             ["alternative", "cash", "different method", "try again"])
    ...
    order_can_finalize = any(phrase in final_twiml.lower() for phrase in 
                            ["finalize", "place", "confirm", "complete"])
