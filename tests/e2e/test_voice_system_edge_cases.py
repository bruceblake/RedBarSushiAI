...
def test_human_agent_transfer_flow(api_request):
    ...
    transfer_acknowledgement = any(
        phrase in transfer_twiml.lower()
        for phrase in ["transfer", "connect", "agent", "representative", "person"]
    )
    ...
    customer_info_preserved = (
        "Taylor" in transfer_twiml or "+15551234567" in transfer_twiml
    )
    if not customer_info_preserved:
        ...
        if "<Redirect>" in transfer_twiml:
            ...
            if "?" in redirect_url:
                ...
                customer_info_preserved = any(
                    [
                        "name" in query_params,
                        "phone" in query_params,
