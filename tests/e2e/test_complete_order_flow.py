...
def test_complete_order_workflow(api_request, create_test_menu_payload):
    ...
    if "data" in menu_payload and "menu" in menu_payload["data"]:
        for cat in menu_payload["data"]["menu"]["categories"]:
            for item in cat["products"]:
                items.append(
                    {"plu": item["plu"], "name": item["name"], "price": item["price"]}
                )
    ...
    for test_item in items:
        ...
        if retrieved_items[test_item["plu"]]["price"] > 100:
            ...
            expected_price = (
                int(test_item["price"] * 100)
                if test_item["price"] < 100
                else test_item["price"]
            )
            ...
            expected_price = (
                test_item["price"]
                if test_item["price"] < 100
                else test_item["price"] / 100
            )
            assert (
                abs(retrieved_items[test_item["plu"]]["price"] - expected_price) < 0.01
            )
    ...
    snoozed_item = next(
        (i for i in updated_menu["items"] if i["plu"] == snooze_item_plu), None
    )
    ...
    complete_response = api_request.post(
        f"/order/{order_id}/status", data=complete_payload
    )
    ...
    unsnooze_payload = {
        ...
        "channelLinkId": "test-channel-link-id",
        "operations": [
            {"action": "unsnooze", "data": {"items": [{"plu": snooze_item_plu}]}}
        ],
        "allSnoozedItems": [],
    }
    ...
    final_item = next(
        (i for i in final_menu["items"] if i["plu"] == snooze_item_plu), None
    )
    assert final_item is not None
    assert (
        final_item.get("snoozed") is not True
        and final_item.get("available") is not False
    )


@pytest.mark.e2e
def test_menu_update_and_retrieval_workflow(
    api_request, deliverect_menu_payload, async_menu_payload
):
    """
    ...
    for category in deliverect_menu_payload["data"]["menu"]["categories"]:
        for item in category["products"]:
            standard_items.append({"plu": item["plu"], "name": item["name"]})
    ...
    for expected_item in standard_items:
        assert any(
            item["plu"] == expected_item["plu"] for item in updated_menu["items"]
        )
    ...
    updated_item = next(
        (i for i in partial_menu["items"] if i["plu"] == standard_items[0]["plu"]), None
    )
    ...
    final_item = next(
        (i for i in final_menu["items"] if i["plu"] == standard_items[0]["plu"]), None
    )
    assert final_item is not None
