...
def test_build_deliverect_order():
            ...
            "modifier": [
                {
                    ...
                    "quantity": 1,
                    "price": 0.50,
                    "reference_handler": "SPICY-MAYO###PRNT",
                }
            ],
        },
        {
    ...
    # Build the order
    payload = build_deliverect_order(
        sender, caller_name, order_items, total_price, order_id
    )
    ...
    assert (
        payload["items"][0]["plu"] == "cal_roll_1"
    )  # Should be cleaned from "cal_roll_1###PRNT"
    assert (
        payload["items"][0]["subItems"][0]["plu"] == "SPICY-MAYO"
    )  # Should be cleaned from "SPICY-MAYO###PRNT"
    ...
    assert payload["payment"]["amount"] == int(
        round(total_price * (1 + sales_tax) * 100)
    )
...
@pytest.mark.skip(
    reason="Skipping order_status_endpoint test as it requires special setup"
)
    ...
    with patch("tasks.send_order_status_update_task") as mock_task:
        ...
        with app.app_context():
            ...
            response = client.post(
                "/order_status",
                json={"channelOrderId": "test-order-123", "status": "DELIVERED"},
            )
            ...
            response = client.post(
                "/order_status",
                json={"channelOrderId": "non-existent", "status": "DELIVERED"},
            )
            ...
            response = client.post("/order_status", json={"status": "DELIVERED"})
...
def test_unique_order_ids_per_location():
    ...
    order_items = [
        {"name": "Item", "quantity": 1, "price": 10.0, "reference_handler": "item1"}
    ]
