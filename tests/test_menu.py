...
def test_is_item_snoozed_timebased():
    ...
    item5 = {"snoozeStart": "invalid", "snoozeEnd": "also invalid"}
...
def test_menu_update_endpoint(
    mock_process, mock_load, mock_write, client, app, setup_test_menu
):
    ...
    test_data = {
        "categories": [
            {"name": "Test Category", "products": [{"name": "Test Product"}]}
        ]
    }
...
def test_snooze_unsnooze_endpoint(mock_write, mock_load, client, app, mock_menu_data):
    ...
    california_roll = next(
        (
            item
            for item in called_data.get("items", [])
