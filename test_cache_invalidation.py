...
def verify_memory_cache_state():
    ...
    print(f"Found {len(menu_item_keys)} menu_item: keys in memory cache: {menu_item_keys}")
...
def run_test():
    ...
    redis_keys_removed = len(redis_before.get('menu_keys', [])) > len(redis_after.get('menu_keys', []))
    redis_item_keys_removed = len(redis_before.get('menu_item_keys', [])) > len(redis_after.get('menu_item_keys', []))
    
    # Memory cache invalidation check
    memory_keys_removed = len(memory_before.get('menu_keys', [])) > len(memory_after.get('menu_keys', []))
    memory_item_keys_removed = len(memory_before.get('menu_item_keys', [])) > len(memory_after.get('menu_item_keys', []))
    ...
    updated_item = next((item for item in updated_menu["items"] 
                        if item["reference_handler"] == "test-sushi-roll"), None)
    ...
    return redis_keys_removed and redis_item_keys_removed and memory_keys_removed and memory_item_keys_removed
